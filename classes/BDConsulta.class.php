...
class BDConsulta {
			...
			case 'postgres':
				...
				if (isset($this->tabela_lista)) {
					if (is_array($this->tabela_lista)) {
						...
						$chave = key($this->tabela_lista); $tabela = current($this->tabela_lista);
						}
	...
	function prepareExcluir() {
		switch (strtolower(trim(config('tipoBd')))) {
			...
			case 'postgres':
				$q = 'DELETE FROM ';
				if (isset($this->tabela_lista)) {
					if (is_array($this->tabela_lista)) { $chave = key($this->tabela_lista); $tabela = current($this->tabela_lista); }
					else $tabela = $this->tabela_lista;
					}
				else return false;
				$q .= $this->_prefixo_tabela.$tabela;
		...
		if ($q = $this->prepare()) {
			if ($depurar) {
				...
				if ($qid) {
					...
					dprint(__file__, __line__, 0, 'Verifica��o SQL: '.var_export($res, true));
					$qid->Close();
					}
				}
			if ($gravar) {
	...
	function inserirObjeto($tabela, &$objeto, $chaveNome = null, $verboso = false) {
		...
		if (!$this->exec())	{
			die('Falhou a inser��o de dados em '.$tabela.': '.$bd->ErrorMsg());
			return false;
			}
