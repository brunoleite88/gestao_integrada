...
class CAplic {
  ...
  var $usuario_posto = null;
    ...
    $this->multiselecaojs_carregado = false;
		...
		$this->gei_logo = BASE_URL.'/'.(isset($config['logotipo']) ? $config['logotipo'] : 'estilo/rondon/imagens/organizacao/'.(isset($config['militar']) ? $config['militar'] : '10').'/gpweb_logo.png');
		$this->gei_brasao = BASE_URL.'/'.(isset($config['brasao']) ? $config['brasao'] : 'estilo/rondon/imagens/brasao.gif');
		}
	...
	function __($str, $estados = 0) {
		...
		// Correção dinâmica: Se não for UTF-8 válido, converte de ISO
	...
	function login($usuarioNome, $senha) {
		...
		if ($outra_conta && isset($outra_conta['usuario_login2']) && $outra_conta['usuario_login2'] && $outra_conta['usuario_senha2']) $this->conta_conjunta=1;
	...
	function mudar_conta($usuario_id) {
		...
		if ($usuario_id==-1){
			//caso de login autom�tico gen�rico
  ...
  function resetJS(){
    ...
    /**
    * Inicializa os campos autom�ticos e tooltip
    */
    ...
    /**
    * Insere o script do combo de multisele��o.
    */
    ...
    */
    ...
    /**
    * Insere os scripts de calend�rio simples (somente data)
    */
    ...
    /**
    * Insere os scripts de calend�rio com data e hora
    */
