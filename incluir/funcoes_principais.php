...
if (!function_exists('toUtf8')) {
    function toUtf8($data) {
        if (is_array($data)) {
            foreach ($data as $k => $v) $data[$k] = toUtf8($v);
            return $data;
        }
...
if (!function_exists('gpw_passthrough')) {
    function gpw_passthrough($data) {
        return $data;
    }
}
	...
	if ($config['email_ativo'] && $config['email_externo_auto']) {
		if ($Aplic->profissional){
			...
			if ($email->EmailValido($contato_email)) {
				$email->Assunto($titulo, ($localidade_tipo_caract ? $localidade_tipo_caract : 'utf-8'));
				...
				$email->Corpo($corpo_email1, ($localidade_tipo_caract ? $localidade_tipo_caract : 'utf-8'));
			...
			$email->Corpo($corpo_email, ($localidade_tipo_caract ? $localidade_tipo_caract : 'utf-8'));
...
function vetor_com_pai_generico($tabela='', $campo_chave='', $campo_nome='', $campo_pai='', $chave_id='', $cia_id=0, $campo_cia='', $mostrar_cia=false, $ajax=FALSE, $campo_acesso='', $tipo='', $vazio='', $diferente_de=false, $filtro=array(), $esqUnir='', $esqOnde=''){
	...
	if ($chave_id) {
		...
		if ($id_pai && $id_pai!=$chave_id){
			...
			$linha=$sql->Linha();
			$sql->limpar();
			if (isset($linha['nome']) && isset($linha[$campo_chave])) $vetor[$linha[$campo_chave]]=($ajax ? gpw_passthrough($linha['nome']) : $linha['nome']);
			else $vetor['']=($ajax ? gpw_passthrough('Retornar à lista superior') : 'Retornar à lista superior');
			}
		else $vetor['']=($ajax ? gpw_passthrough('Retornar à lista superior') : 'Retornar à lista superior');
		$espacamento='&nbsp;&nbsp;';
		}
	...
	if ($chave_id){
		...
		$linha=$sql->Linha();
		$sql->limpar();
		if (isset($linha['nome']) && isset($linha[$campo_chave])) $vetor[$linha[$campo_chave]]=$espacamento.($ajax ? gpw_passthrough($linha['nome']) : $linha['nome']);
		}
	...
	foreach((array)$lista as $linha) {
		...
		if ($permite) $vetor[$linha[$campo_chave]]=$espacamento.($chave_id ? '&nbsp;&nbsp;&nbsp;' : '').($ajax ? gpw_passthrough($linha['nome']) : $linha['nome']);
		}
	return $vetor;
	}
...
function vetor_campo_sistema($campo='', $chave_id='', $ajax=FALSE, $projeto_id=null){
	$sql = new BDConsulta;
	$vetor=array();
	if ($chave_id) $vetor['']=($ajax ? gpw_passthrough('Retornar à lista superior') : 'Retornar à lista superior');
	else $vetor['']='';
	if ($chave_id){
		...
		$linha=$sql->Linha();
		$sql->limpar();
		if (isset($linha['sisvalor_valor_id']) && isset($linha['sisvalor_valor'])) $vetor[$linha['sisvalor_valor_id']]=($ajax ? gpw_passthrough($linha['sisvalor_valor']) : $linha['sisvalor_valor']);
		}
	$sql->adTabela('sisvalores');
	$sql->adCampo('sisvalor_valor_id, sisvalor_valor');
	...
	$lista=$sql->Lista();
	$sql->limpar();
	foreach((array)$lista as $linha) $vetor[$linha['sisvalor_valor_id']]=($chave_id ? '&nbsp;&nbsp;&nbsp;' : '').($ajax ? gpw_passthrough($linha['sisvalor_valor']) : $linha['sisvalor_valor']);
	return $vetor;
	}
...
function vetor_nd($nd_item_subitem='', $ajax=FALSE, $projeto_id=null, $nd_classe=3, $nd_grupo='', $nd_subgrupo='', $nd_elemento_subelemento='', $chave_nd_id=false){
	...
	foreach($lista as $linha) $vetor[$linha['sisvalor_valor_id']]=($ajax ? gpw_passthrough($linha['sisvalor_valor']) : $linha['sisvalor_valor']);
	return $vetor;
	}
...
function vetor_chavepai($tipo='', $chave='', $ajax=FALSE, $permite_vazio=false, $projeto_id=null){
	...
	if($chave){
		...
		$vetor[$atual['sisvalor_valor_id']]=$espaco.($ajax ? gpw_passthrough($atual['sisvalor_valor']) : $atual['sisvalor_valor']);
		}
	...
	$lista=$sql->Lista();
	$sql->limpar();
	foreach($lista as $linha) $vetor[$linha['sisvalor_valor_id']]=$espaco.'&nbsp;&nbsp;&nbsp;'.($ajax ? gpw_passthrough($linha['sisvalor_valor']) : $linha['sisvalor_valor']);
	return $vetor;
	}
...
global $Aplic, $config;
	...
	if (!$Aplic->usuario_pode_outra_cia && !$Aplic->usuario_pode_superior && !$Aplic->usuario_pode_lateral && !$externo && !$administrador){
		...
		if ($vazio) $vetor[null]=($ajax ? gpw_passthrough($vazio) : $vazio);
		$vetor[$Aplic->usuario_cia]=($ajax ? gpw_passthrough($cia_nome) : $cia_nome);
	...
	if (!$cia_id) {
		...
		$sql->limpar();
		$vetor[$Aplic->usuario_cia]=($ajax ? gpw_passthrough($nome_cia) : $nome_cia);
	...
	if ($acesso) $vetor[$cia_superior['cia_id_superior']]=($ajax ? gpw_passthrough($cia_superior['nome_superior']) : $cia_superior['nome_superior']);
	$vetor[$cia_superior['cia_id']]='&nbsp;&nbsp;'.($ajax ? gpw_passthrough($cia_superior['nome']) : $cia_superior['nome']);
	...
	if ($administrador || $cia_id==$cia_superior_usuario || $Aplic->usuario_pode_outra_cia){
		...
		$linhas=$sql->Lista();
		$sql->limpar();
		foreach($linhas as $linha)$vetor[$linha['cia_id']]='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.($ajax ? gpw_passthrough($linha['nome']) : $linha['nome']);
		}
...
function selecionar_cidades_para_ajax($estado_sigla='', $campo, $script, $vazio='', $cidade='', $id_municipio=false, $ajax=true){
	...
	if ($id_municipio){
		...
		$vetor=array();
		$vetor['']='';
		if ($ajax) foreach($cidades as $linha) $vetor[gpw_passthrough($linha['municipio_id'])]=gpw_passthrough($linha['municipio_nome']);
		...
		$vetor=array();
		$vetor['']='';
		if ($ajax) foreach($cidades as $linha) $vetor[gpw_passthrough($linha['municipio_nome'])]=gpw_passthrough($linha['municipio_nome']);
...
function mudar_usuario_para_ajax($cia_id=0, $usuario_id=0, $campo='', $posicao='', $script='', $segunda_tabela='', $condicao='', $mostrar_cia=false, $mostrar_funcao=false){
	...
	$vetor=array();
	$vetor[0]='';
	foreach((array)$linhas as $linha)	$vetor[$linha['usuario_id']]=gpw_passthrough($linha['nome_usuario'].($mostrar_funcao && $linha['contato_funcao'] ? ' - '.$linha['contato_funcao']: '').($mostrar_cia && $linha['cia_nome'] ? ' - '.$linha['cia_nome']: ''));
	if (count($vetor)==1) $vetor[-1]='';
...
function mudar_usuario_em_dept($ajax=false, $cia_id=0, $usuario_id=0, $campo='', $posicao='', $script='', $segunda_tabela='', $condicao='', $mostrar_cia=false, $mostrar_funcao=false){
	...
	foreach ($depts as $dept_id => $secao_data){
		$vetor[--$qnt]=($ajax ? gpw_passthrough($secao_data['dept_nome']) : $secao_data['dept_nome']);
		...
		foreach ($linhas as $linha) {
			...
			$vetor[$linha['usuario_id']]='&nbsp;&nbsp;&nbsp;'.($ajax ? gpw_passthrough(nome_funcao(($config['militar'] < 10 ? $linha['contato_posto'].' '.$linha['contato_nomeguerra'] : $linha['contato_nomeguerra']),'',$linha['contato_funcao'])) : nome_funcao(($config['militar'] < 10 ? $linha['contato_posto'].' '.$linha['contato_nomeguerra'] : $linha['contato_nomeguerra']),'',$linha['contato_funcao']));
	...
	$sql->Limpar();
	if (count($usuarios)){
		$vetor[--$qnt]=($ajax ? gpw_passthrough('Em '.($config['genero_dept']=='o' ? 'nenhum ': 'nenhuma ').' '.strtolower($config['departamento'])) : 'Em '.($config['genero_dept']=='o' ? 'nenhum ': 'nenhuma ').' '.strtolower($config['departamento']));
		$estilo[$qnt]='font-weight:bold;';
		foreach ($usuarios as $usuario) {
			$nome=
			$vetor[$usuario['usuario_id']]='&nbsp;&nbsp;&nbsp;'.($ajax ? gpw_passthrough(nome_funcao(($config['militar'] < 10 ? $usuario['contato_posto'].' '.$usuario['contato_nomeguerra'] : $usuario['contato_nomeguerra']),'',$usuario['contato_funcao'])) : nome_funcao(($config['militar'] < 10 ? $usuario['contato_posto'].' '.$usuario['contato_nomeguerra'] : $usuario['contato_nomeguerra']),'',$usuario['contato_funcao']));
...
function mudar_usuario_em_dept_subniveis($dept_id, $subnivel, &$vetor, &$qnt, &$estilo, $ajax){
	...
	foreach($subordinados as $linha){
		$vetor[--$qnt]=$subnivel.($ajax ? gpw_passthrough($linha['dept_nome']) : $linha['dept_nome']);
		$estilo[$qnt]='font-weight:bold;';
		...
		$sql->Limpar();
		foreach ($usuarios as $usuario) {
			$vetor[$usuario['usuario_id']]='&nbsp;&nbsp;&nbsp;'.$subnivel.($ajax ? gpw_passthrough(nome_funcao(($config['militar'] < 10 ? $usuario['contato_posto'].' '.$usuario['contato_nomeguerra'] : $usuario['contato_nomeguerra']),'',$usuario['contato_funcao'])) : nome_funcao(($config['militar'] < 10 ? $usuario['contato_posto'].' '.$usuario['contato_nomeguerra'] : $usuario['contato_nomeguerra']),'',$usuario['contato_funcao']));
			$estilo[$usuario['usuario_id']]='font-weight:normal;';
			}
...
function mjson_decode($json){
  ...
  return json_decode(removerVirgulas(gpw_passthrough($json)), true);
  }
...
function formSeguro($txt, $tira_barra = false) {
	global $localidade_tipo_caract;
	if ($txt=='') return '';
	if (!$localidade_tipo_caract) $localidade_tipo_caract = 'utf-8';
	if (is_object($txt)) {
