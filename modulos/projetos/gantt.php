...
if (is_array($projetos)) {
	foreach ($projetos as $p) {
		if ($localidade_tipo_caract == 'utf-8' && function_exists('utf8_decode')) $nome = strlen(gpw_passthrough($p['projeto_nome'])) > 25 ? substr(gpw_passthrough($p['projeto_nome']), 0, 22).'...' : gpw_passthrough($p['projeto_nome']);
