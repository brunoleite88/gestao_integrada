...
function inserir_acao($ata_acao_id, $ata_id, $ata_acao_texto=null, $data=null, $ata_acao_responsavel=null){
	...
	if (!$ata_acao_id){
	  ...
	  $sql->Limpar();
		...
		$sql->adInserir('ata_acao_texto', previnirXSS(gpw_passthrough($ata_acao_texto)));
		...
		$sql->adTabela('ata_acao');
		$sql->adAtualizar('ata_acao_texto', previnirXSS(gpw_passthrough($ata_acao_texto)));
...
function lista_artefatos($ata_id){
	...
	if ($acaos && count($acaos)) $saida.= '<table class="tbl1" cellspacing=0 cellpadding=0 border=0 width="100%"><tr><th></th><th>&nbsp;'.gpw_passthrough('Ações').'&nbsp;</th><th>&nbsp;Data Limite&nbsp;</th><th>&nbsp;'.gpw_passthrough('Responsável').'&nbsp;</th><th></th></tr>';
	foreach ($acaos as $acao) {
		...
		$saida.='</td>';
		$saida.='<td>&nbsp;'.gpw_passthrough($acao['ata_acao_texto']).'&nbsp;</td>';
		$saida.='<td>&nbsp;'.gpw_passthrough(retorna_data($acao['ata_acao_fim'])).'&nbsp;</td>';
		$saida.='<td>&nbsp;'.gpw_passthrough(nome_funcao('','','','',$acao['ata_acao_responsavel'])).'&nbsp;</td>';
		$saida.='<td width="32" align="center"><a href="javascript: void(0);" onclick="editar_acao('.$acao['ata_acao_id'].');">'.imagem('icones/editar.gif').'</a>';
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta ação da ata de reunião?').'\')) {excluir_acao('.$acao['ata_acao_id'].');}">'.imagem('icones/remover.png').'</a></td>';
		$saida.='</tr>';
		}
...
function editar_acao($ata_acao_id){
	...
	$saida='<table cellpadding=0 cellspacing="2"><tr><td><b>'.gpw_passthrough('Ação').'</b></td><td><b>Data limite</b></td><td><b>'.gpw_passthrough('Responsável').'</b></td><td></td></tr><tr>';
	$saida.='<td valign=top><textarea name="ata_acao_texto" id="ata_acao_texto" style="width:600px;" class="textarea">'.gpw_passthrough($acao['ata_acao_texto']).'</textarea></td>';
	...
	$saida.='<td valign=top><table cellspacing=0 cellpadding=0><tr><td valign=top><input type="hidden" id="ata_acao_responsavel" name="ata_acao_responsavel" value="'.$acao['ata_acao_responsavel'].'" /><input type="text" id="nome_responsavel" name="nome_responsavel" value="'.gpw_passthrough($acao['nome']).'" style="width:200px;" class="texto" READONLY /></td><td valign=top><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif').'</a></td></tr></table></td>';
...
function cancelar_edicao(){
	...
	$saida.='<tr><td><b>'.gpw_passthrough('Ação').'</b></td><td><b>'.gpw_passthrough('Data limite').'</b></td><td><b>'.gpw_passthrough('Responsável').'</b></td><td>&nbsp;</td></tr>';
