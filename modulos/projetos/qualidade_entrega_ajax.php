...
function inserir_entrega($projeto_qualidade_entrega_id, $projeto_id, $projeto_qualidade_entrega_entrega, $projeto_qualidade_entrega_criterio ){
	...
	if (!$projeto_qualidade_entrega_id){
	  ...
	  $sql->Limpar();
		...
		$sql->adInserir('projeto_qualidade_entrega_entrega', previnirXSS(gpw_passthrough($projeto_qualidade_entrega_entrega)));
		$sql->adInserir('projeto_qualidade_entrega_ordem', $soma_total);
		$sql->adInserir('projeto_qualidade_entrega_criterio', previnirXSS(gpw_passthrough($projeto_qualidade_entrega_criterio)));
		...
		$sql->adTabela('projeto_qualidade_entrega');
		$sql->adAtualizar('projeto_qualidade_entrega_entrega', previnirXSS(gpw_passthrough($projeto_qualidade_entrega_entrega)));
		$sql->adAtualizar('projeto_qualidade_entrega_criterio', previnirXSS(gpw_passthrough($projeto_qualidade_entrega_criterio)));
...
function lista_artefatos($projeto_id){
	...
	if ($entregas && count($entregas)) $saida.= '<table class="tbl1" cellspacing=0 cellpadding=0 border=0 width="100%"><tr><th></th><th>&nbsp;Entrega'.(count($entregas)>1 ? 's':'').'&nbsp;</th><th>&nbsp;'.gpw_passthrough('Critério').(count($entregas)>1 ? 's':'').' de qualidade&nbsp;</th><th></th></tr>';
	foreach ($entregas as $entrega) {
		...
		$saida.='</td>';
		$saida.='<td>&nbsp;'.gpw_passthrough($entrega['projeto_qualidade_entrega_entrega']).'&nbsp;</td><td>&nbsp;'.gpw_passthrough($entrega['projeto_qualidade_entrega_criterio']).'&nbsp;</td>';
		...
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta entrega e critérios de qualidade?').'\')) {excluir_entrega('.$entrega['projeto_qualidade_entrega_id'].');}">'.imagem('icones/remover.png').'</a></td>';
		$saida.='</tr>';
		}
...
function editar_entrega($projeto_qualidade_entrega_id){
	...
	$sql->adOnde('projeto_qualidade_entrega_id='.$projeto_qualidade_entrega_id);
	$entrega=$sql->Linha();
	$saida.='<table cellpadding=0 cellspacing="2"><tr><td><b>Entrega</b></td><td><b>'.gpw_passthrough('Critérios').' de qualidade</b></td><td></td></tr>';
	$saida.='<tr><td valign=top><input type="text" name="projeto_qualidade_entrega_entrega" id="projeto_qualidade_entrega_entrega" size="50" class="texto" value="'.gpw_passthrough($entrega['projeto_qualidade_entrega_entrega']).'" /></td>';
	$saida.='<td valign=top><textarea name="projeto_qualidade_entrega_criterio" id="projeto_qualidade_entrega_criterio" style="width:500px;" class="textarea">'.gpw_passthrough($entrega['projeto_qualidade_entrega_criterio']).'</textarea></td>';
