...
function exibir_contatos($contatos){
	...
	$objResposta->assign('combo_contatos',"innerHTML", gpw_passthrough($saida_contatos));
	return $objResposta;				
	}
...
function exibir_municipios($municipios){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign('combo_municipios',"innerHTML", gpw_passthrough($saida_municipios));
	return $objResposta;				
	}
...
function exibir_cias($cias){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign('combo_cias',"innerHTML", gpw_passthrough($saida_cias));
	return $objResposta;				
	}
...
function projeto_existe($nome='', $projeto_id=0){
	$nome=previnirXSS(gpw_passthrough($nome));
	$sql = new BDConsulta;
...
function acao_ajax($social_id=0){
	...
	$lista=$sql->Lista();
	$sql->limpar();
	foreach ($lista as $linha) $lista_acoes[$linha['social_acao_id']]=gpw_passthrough($linha['social_acao_nome']);
...
function selecionar_comunidade_ajax($municipio_id='', $campo='', $posicao='', $script='', $vazio='', $projeto_comunidade=0){
	...
	$vetor=array();
	$vetor['']=$vazio;
	foreach($comunidades as $linha) $vetor[gpw_passthrough($linha['social_comunidade_id'])]=gpw_passthrough($linha['social_comunidade_nome']);
...
function incluir_envolvido_ajax($projeto_id=0, $uuid='', $contato_id, $envolvimento, $perfil){
	$envolvimento=previnirXSS(gpw_passthrough($envolvimento));
	$perfil=previnirXSS(gpw_passthrough($perfil));
	$sql = new BDConsulta;
...
function atualizar_envolvidos($projeto_id=0, $uuid=''){
	...
	if (count($contatos)) {
		...
		$saida.= '<tr><th></th><th>Nome</th><th>'.gpw_passthrough($config['organizacao']).'</th><th>'.gpw_passthrough('Função').'</th><th>'.gpw_passthrough('Relevância').'</th><th>'.gpw_passthrough('Característica/Perfil').'</th><th></th></tr>';
		foreach ($contatos as $contato_id => $contato_data) {
			...
			$saida.= '</td>';
			$saida.= '<td align="left" nowrap="nowrap">'.gpw_passthrough($contato_data['nome_contato']).'</td>';
			$saida.= '<td align="left">'.gpw_passthrough($contato_data['cia_nome']).'</td>';
			$saida.= '<td align="left">'.gpw_passthrough($contato_data['contato_funcao']).'</td>';
			$saida.= '<td align="left">'.gpw_passthrough($contato_data['envolvimento']).'</td>';
			$saida.= '<td align="left">'.gpw_passthrough($contato_data['perfil']).'</td>';
...
function mudar_posicao_integrante_ajax($ordem, $projeto_integrantes_id, $direcao, $projeto_id=0, $uuid=''){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign("integrantes","innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function incluir_integrante_ajax($projeto_id=0, $uuid='', $contato_id, $projeto_integrante_competencia, $projeto_integrante_atributo='', $projeto_integrantes_situacao='', $projeto_integrantes_necessidade=''){
	$sql = new BDConsulta;
	$projeto_integrante_competencia=previnirXSS(gpw_passthrough($projeto_integrante_competencia));
	$projeto_integrante_atributo=previnirXSS(gpw_passthrough($projeto_integrante_atributo));
	$projeto_integrantes_situacao=previnirXSS(gpw_passthrough($projeto_integrantes_situacao));
	$projeto_integrantes_necessidade=previnirXSS(gpw_passthrough($projeto_integrantes_necessidade));
	//verificar se já existe
	$sql->adTabela('projeto_integrantes');
...
function editar_integrante($projeto_integrantes_id){
	...
	$objResposta->assign("nome_integrante","value", gpw_passthrough($nome));
	$objResposta->assign("integrante_id","value", $linha['contato_id']);	
	$objResposta->assign("projeto_integrante_atributo","value", gpw_passthrough($linha['projeto_integrante_atributo']));	
	$objResposta->assign("apoio1","value", gpw_passthrough($linha['projeto_integrante_atributo']));	
	
	$objResposta->assign("projeto_integrantes_situacao","value", gpw_passthrough($linha['projeto_integrantes_situacao']));	
	$objResposta->assign("apoio2","value", gpw_passthrough($linha['projeto_integrantes_situacao']));	
	
	$objResposta->assign("projeto_integrantes_necessidade","value", gpw_passthrough($linha['projeto_integrantes_necessidade']));	
	$objResposta->assign("apoio3","value", gpw_passthrough($linha['projeto_integrantes_necessidade']));	
	return $objResposta;
	}	
...
function editar_envolvido($projeto_contato_id){
	...
	$objResposta->assign("nome_envolvido","value", gpw_passthrough($nome));
	$objResposta->assign("envolvimento","value", gpw_passthrough($linha['envolvimento']));
	$objResposta->assign("envolvido_id","value", $linha['contato_id']);	
	$objResposta->assign("perfil","value", gpw_passthrough($linha['perfil']));	
	$objResposta->assign("apoio1","value", gpw_passthrough($linha['perfil']));	
	return $objResposta;
	}	
