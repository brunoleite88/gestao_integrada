...
function ver_debug($vetor){
  ...
  $objResposta->assign('combo_debug',"innerHTML",gpw_passthrough($vetor));
  return $objResposta;
  }
...
function renomear_tarefa_pro($tarefa_id=0, $nome){
  ...
  $sql->adAtualizar('tarefa_nome', previnirXSS(gpw_passthrough($nome)));
...
function renomear_projeto_pro($projeto_id=0, $nome){
  ...
  $sql->adTabela('projetos');
  $sql->adAtualizar('projeto_nome', previnirXSS(gpw_passthrough($nome)));
...
function criarProjeto($cia_id=0, $nome_projeto='', $usuario_id=0){
  ...
  $sql = new BDConsulta;
  $sql->adTabela('projetos');
  $sql->adInserir('projeto_nome', previnirXSS(gpw_passthrough($nome_projeto)));
...
function inserir_tarefa($tarefa_projeto=0, $tarefa_superior=0, $nome='', $tarefa_pai=0){
  ...
  $sql->adTabela('tarefas');
  $sql->adInserir('tarefa_nome', previnirXSS(gpw_passthrough($nome)));
...
function exibir_tarefas($projeto_id=0, $por_numeracao=0, $tarefa_id=0){
  global $Aplic, $vetor_tarefas;
    ...
    $saida.='<tr><td colspan=20 align=center><b>Projeto</b>&nbsp;<input class="texto" type="Text" '.(!$podeEditarProjeto ? 'READONLY': '' ).' onchange="mudar_nome_projeto(\''.$projeto_id.'\')" style="width:180px;" id="pro'.$projeto_id.'" value="'.gpw_passthrough($projeto['projeto_nome']).'" onchange="mudar_nome_projeto(\''.$projeto_id.'\')" onFocus="tarefaAtiva(-1)" />'.($podeEditarProjeto ? '<img src="./estilo/rondon/imagens/icones/editar.gif" onclick="javascript:editar_projeto('.$projeto_id.')" style="cursor:pointer; vertical-align:middle"/>': '' ).'</td></tr>';
    ...
    if (count($tarefas)) $saida.='<tr>'.($Aplic->profissional ? '<th>&nbsp;</th><th>'.gpw_passthrough('Nº').'</th>':'').'<th>Tarefa</th><th>'.gpw_passthrough('Início').'</th><th>'.gpw_passthrough('Término').'</th><th>Dias</th><th>'.gpw_passthrough('Predecessoras').'</th><th>'.gpw_passthrough('%').'</th><th></th></tr>';
    foreach($tarefas as $tarefa){
