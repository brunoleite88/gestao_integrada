...
function inserir_pauta($ata_pauta_id, $ata_pauta_ata, $ata_pauta_texto, $tipo){
	...
	if (!$ata_pauta_id){
	  ...
	  $sql->Limpar();
		...
		$sql->adInserir('ata_pauta_texto', previnirXSS(gpw_passthrough($ata_pauta_texto)));
		$sql->adInserir('ata_pauta_ordem', $soma_total);
		$sql->adInserir('ata_pauta_tipo', previnirXSS(gpw_passthrough($tipo)));
		$sql->exec();
		$sql->Limpar();
		}
	else{
		$sql->adTabela('ata_pauta');
		$sql->adAtualizar('ata_pauta_texto', previnirXSS(gpw_passthrough($ata_pauta_texto)));
...
function lista_artefatos($ata_pauta_ata,$tipo){
	...
	foreach ($pautas as $pauta) {
		...
		$saida.='</td>';
		$saida.='<td>&nbsp;'.gpw_passthrough($pauta['ata_pauta_texto']).'&nbsp;</td>';
		...
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta pauta de ata de reunião?').'\')) {excluir_pauta('.$pauta['ata_pauta_id'].');}">'.imagem('icones/remover.png').'</a></td>';
		$saida.='</tr>';
		}
...
function editar_pauta($ata_pauta_id){
	...
	$saida.='<tr><td valign=top><textarea name="ata_pauta_texto" id="ata_pauta_texto" style="width:750px;" class="textarea">'.gpw_passthrough($pauta['ata_pauta_texto']).'</textarea></td>';
