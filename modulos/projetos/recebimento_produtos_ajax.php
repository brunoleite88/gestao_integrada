...
function inserir_tipo($projeto_recebimento_lista_id, $projeto_recebimento_lista_recebimento_id, $projeto_recebimento_lista_produto=null){
	...
	if (!$projeto_recebimento_lista_id){
	  ...
	  $sql->Limpar();
		...
		$sql->adInserir('projeto_recebimento_lista_produto', previnirXSS(gpw_passthrough($projeto_recebimento_lista_produto)));
		$sql->exec();
		$sql->Limpar();
		}
	else{
		...
		$sql->adAtualizar('projeto_recebimento_lista_produto', previnirXSS(gpw_passthrough($projeto_recebimento_lista_produto)));
...
function lista_artefatos($projeto_recebimento_lista_recebimento_id){
	...
	foreach ($tipos as $tipo) {
		...
		$saida.='<td>'.($tipo['projeto_recebimento_lista_produto'] ? gpw_passthrough($tipo['projeto_recebimento_lista_produto']) : '&nbsp;').'</td>';
		...
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir este produto/serviço?').'\')) {excluir_tipo('.$tipo['projeto_recebimento_lista_id'].');}">'.imagem('icones/remover.png').'</a></td>';
		$saida.='</tr>';
		}
...
function editar_tipo($projeto_recebimento_lista_id){
	...
	$saida.='<td></td></tr><tr>';
	$saida.='<td valign=top><textarea name="projeto_recebimento_lista_produto" id="projeto_recebimento_lista_produto" class="textarea" style="width:100%">'.gpw_passthrough($tipo['projeto_recebimento_lista_produto']).'</textarea></td>';
