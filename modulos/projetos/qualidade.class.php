...
class CQualidade extends CAplicObjeto {
  var $projeto_qualidade_projeto = null;
  var $projeto_qualidade_usuario = null;
  var $projeto_qualidade_descricao = null;
  var $projeto_qualidade_data = null;
	...
	function armazenar($atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($_REQUEST['antigo']) {
			$ret = $sql->atualizarObjeto('projeto_qualidade', $this, 'projeto_qualidade_projeto', false);
			$sql->limpar();
			}
		else {
	...
	function notificar($post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo == 'excluido') {
					$email->Assunto('Excluído qualidade do projeto', $localidade_tipo_caract);
					$titulo='Excluído qualidade do projeto';
					}
				elseif ($tipo=='atualizado') {
					$email->Assunto('Atualizado qualidade do projeto', $localidade_tipo_caract);
					$titulo='Atualizado qualidade do projeto';
					}
				else {
					$email->Assunto('Inserido qualidade do projeto', $localidade_tipo_caract);
					$titulo='Inserido qualidade do projeto';
					}
				if ($tipo=='atualizado') $corpo = 'Atualizado qualidade do projeto: '.$projeto_nome.'<br>';
				elseif ($tipo=='excluido') $corpo = 'Excluído qualidade do projeto: '.$projeto_nome.'<br>';
				else $corpo = 'Inserido qualidade do projeto: '.$projeto_nome.'<br>';

				if ($tipo!='excluido') $corpo .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=viabilidade_ver&projeto_qualidade_projeto='.$this->projeto_qualidade_projeto.'\');"><b>Clique para acessar o qualidade do projeto</b></a>';

				if ($tipo=='excluido') $corpo .= '<br><br><b>Responsável pela exclusão do qualidade do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				elseif ($tipo=='atualizado') $corpo .= '<br><br><b>Responsável pela edição do qualidade do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				else $corpo .= '<br><br><b>Criador do qualidade do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
