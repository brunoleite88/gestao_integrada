...
function excluir_calendario($projeto_comunicacao_evento_id=null, $evento_id=null){
  ...
  $saida='';
	...
	foreach ($lista as $linha){
		...
		$saida.='<td colspan=6><a href="javascript: void(0);" onclick="pop_evento('.$linha['evento_id'].');">'.$linha['inicio'].' - '.$linha['fim'].' - '.gpw_passthrough($linha['evento_titulo']).'</a></td>';
		...
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta evento de calendário do plano de comunicação?').'\')) {excluir_calendario('.$projeto_comunicacao_evento_id.', '.$linha['evento_id'].');}">'.imagem('icones/remover.png').'</a>';
		$saida.='</td>';
		$saida.='</tr>';
		}
...
function inserir_evento($projeto_comunicacao_evento_id, $projeto_id, $projeto_comunicacao_evento_evento, $projeto_comunicacao_evento_objetivo, $projeto_comunicacao_evento_responsavel, $projeto_comunicacao_evento_publico, $projeto_comunicacao_evento_canal, $projeto_comunicacao_evento_periodicidade){
	...
	if (!$projeto_comunicacao_evento_id){
	  ...
	  $sql->Limpar();
		...
		$sql->adInserir('projeto_comunicacao_evento_evento', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_evento)));
		$sql->adInserir('projeto_comunicacao_evento_objetivo', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_objetivo)));
		$sql->adInserir('projeto_comunicacao_evento_responsavel', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_responsavel)));
		$sql->adInserir('projeto_comunicacao_evento_publico', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_publico)));
		$sql->adInserir('projeto_comunicacao_evento_canal', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_canal)));
		$sql->adInserir('projeto_comunicacao_evento_periodicidade', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_periodicidade)));
		$sql->exec();
		$sql->Limpar();
		}
	else{
		$sql->adTabela('projeto_comunicacao_evento');
		$sql->adAtualizar('projeto_comunicacao_evento_evento', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_evento)));
		$sql->adAtualizar('projeto_comunicacao_evento_objetivo', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_objetivo)));
		$sql->adAtualizar('projeto_comunicacao_evento_responsavel', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_responsavel)));
		$sql->adAtualizar('projeto_comunicacao_evento_publico', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_publico)));
		$sql->adAtualizar('projeto_comunicacao_evento_canal', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_canal)));
		$sql->adAtualizar('projeto_comunicacao_evento_periodicidade', previnirXSS(gpw_passthrough($projeto_comunicacao_evento_periodicidade)));
...
function lista_artefatos($projeto_id){
	...
	if ($eventos && count($eventos)) {
		...
		$saida.='<th>'.gpw_passthrough('Responsável').'</th>';
		$saida.='<th>'.gpw_passthrough('Público alvo').'</th>';
		$saida.='<th>Canal</th>';
	...
	foreach ($eventos as $evento) {
		...
		$saida.='</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_evento'] ? gpw_passthrough($evento['projeto_comunicacao_evento_evento']) : '&nbsp;').'</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_objetivo'] ? gpw_passthrough($evento['projeto_comunicacao_evento_objetivo']) : '&nbsp;').'</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_responsavel'] ? gpw_passthrough($evento['projeto_comunicacao_evento_responsavel']) : '&nbsp;').'</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_publico'] ? gpw_passthrough($evento['projeto_comunicacao_evento_publico']) : '&nbsp;').'</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_canal'] ? gpw_passthrough($evento['projeto_comunicacao_evento_canal']) : '&nbsp;').'</td>';
		$saida.='<td>'.($evento['projeto_comunicacao_evento_periodicidade'] ? gpw_passthrough($evento['projeto_comunicacao_evento_periodicidade']) : '&nbsp;').'</td>';
		...
		$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta evento do plano de comunicação?').'\')) {excluir_evento('.$evento['projeto_comunicacao_evento_id'].');}">'.imagem('icones/remover.png').'</a>';
		$saida.='</td>';
		$saida.='</tr>';
		...
		if ($Aplic->profissional){
		  ...
		  $sql->Limpar();
			...
			foreach ($lista as $linha){
				...
				$saida.='<a href="javascript: void(0);" onclick="if (confirm(\''.gpw_passthrough('Tem certeza que deseja excluir esta evento de calendário do plano de comunicação?').'\')) {excluir_calendario('.$evento['projeto_comunicacao_evento_id'].', '.$linha['evento_id'].');}">'.imagem('icones/remover.png').'</a>';
				$saida.='</td>';
				$saida.='</tr>';
				}
...
function editar_evento($projeto_comunicacao_evento_id){
	...
	$objResposta->assign("projeto_comunicacao_evento_evento","value", gpw_passthrough($evento['projeto_comunicacao_evento_evento']));
	$objResposta->assign("projeto_comunicacao_evento_objetivo","value", gpw_passthrough($evento['projeto_comunicacao_evento_objetivo']));
	$objResposta->assign("projeto_comunicacao_evento_responsavel","value", gpw_passthrough($evento['projeto_comunicacao_evento_responsavel']));
	$objResposta->assign("projeto_comunicacao_evento_publico","value", gpw_passthrough($evento['projeto_comunicacao_evento_publico']));
	$objResposta->assign("projeto_comunicacao_evento_canal","value", gpw_passthrough($evento['projeto_comunicacao_evento_canal']));
	$objResposta->assign("projeto_comunicacao_evento_periodicidade","value", gpw_passthrough($evento['projeto_comunicacao_evento_periodicidade']));
