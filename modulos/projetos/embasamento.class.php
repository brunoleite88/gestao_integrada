...
class CEmbasamento extends CAplicObjeto {
  var $projeto_embasamento_projeto = null;
  var $projeto_embasamento_responsavel = null;
  var $projeto_embasamento_justificativa = null;
  var $projeto_embasamento_objetivo = null;
  var $projeto_embasamento_escopo = null;
  var $projeto_embasamento_nao_escopo = null;
  var $projeto_embasamento_premissas = null;
  var $projeto_embasamento_restricoes = null;
  var $projeto_embasamento_orcamento = null;
  var $projeto_embasamento_data = null;
	...
	function armazenar($atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($_REQUEST['antigo']) {
			$ret = $sql->atualizarObjeto('projeto_embasamento', $this, 'projeto_embasamento_projeto', false);
			$sql->limpar();
			}
		else {
	...
	function notificar($post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo!='excluido') $corpo .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=viabilidade_ver&projeto_embasamento_projeto='.$this->projeto_embasamento_projeto.'\');"><b>Clique para acessar o embasamento do projeto</b></a>';
