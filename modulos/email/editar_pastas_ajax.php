...
function atualizar_pastas($usuario_id){
	...
	$saida='';
	if (count($pastas)) {
		$saida.= '<table cellspacing=0 cellpadding=0><tr><td></td><td><table cellpadding=0 cellspacing=0 class="tbl1" align=left><tr><th></th><th>'.gpw_passthrough('Nome').'</th><th></th></tr>';
		foreach ($pastas as $pasta_id => $linha) {
			...
			$saida.= '</td>';
			$saida.= '<td align="left">'.gpw_passthrough($linha['nome']).'</td>';
...
function editar_pasta($pasta_id){
	...
	$objResposta->assign("nome","value", gpw_passthrough($linha['nome']));	
	return $objResposta;
	}	
