...
function incluir_grupo_ajax($grupo_usuario=0, $grupo_id=0, $grupo_descricao='', $usuarios=''){
	...
	$sql = new BDConsulta;
	$grupo_descricao=previnirXSS(gpw_passthrough($grupo_descricao));
...
function atualizar_grupos($grupo_usuario){
	...
	if (count($grupos)) {
		...
		foreach ($grupos as $grupo_id => $linha) {
			...
			$saida.= '</td>';
			$saida.= '<td align="left">'.gpw_passthrough($linha['grupo_descricao']).'</td>';
...
function editar_grupo($grupo_id){
	...
	$objResposta->assign('combo_para',"innerHTML", gpw_passthrough($saida));
	$objResposta->assign("grupo_id","value", $grupo_id);
	$objResposta->assign("grupo_descricao","value", gpw_passthrough($linha['grupo_descricao']));	
	return $objResposta;
	}	
