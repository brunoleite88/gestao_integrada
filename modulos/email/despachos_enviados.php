...
$sql->adCampo('msg_usuario.msg_usuario_id, anotacao.anotacao_id, anotacao.datahora, data_retorno, resposta_despacho, texto, data_limite, de_id');
if ($tab==1 || $tab==3) $sql->adOnde('msg_usuario.de_id = '.$Aplic->usuario_id);
if ($tab==0 || $tab==2) $sql->adOnde('msg_usuario.para_id = '.$Aplic->usuario_id);
...
if ($onde) $sql->adOnde('(anotacao.texto LIKE \'%'.$onde.'%\' OR anotacao.nome_de LIKE \'%'.$onde.'%\' OR anotacao.funcao_de LIKE \'%'.$onde.'%\' OR msg_usuario.nome_para LIKE \'%'.$onde.'%\' OR msg_usuario.funcao_para LIKE \'%'.$onde.'%\' OR nota LIKE \'%'.$onde.'%\' OR resposta_despacho LIKE \'%'.$onde.'%\')');
$sql->adOnde('anotacao.anotacao_id IS NOT NULL');
$sql->adGrupo('msg_usuario.msg_usuario_id, anotacao.anotacao_id, anotacao.datahora, data_retorno, resposta_despacho, texto, data_limite, de_id');
...
echo '<input type=hidden name="a" id="a" value="lista_despacho">';
...
echo '<tr><td>'.botao('salvar', 'Salvar','Clique neste botão para salvar '.($tab==0 ? 'a resposta ao despacho.' : $config['genero_mensagem'].' '.$config['mensagem']).'.','','frmdespacho.submit();').'</td><td align=right>'.botao('cancelar', 'Cancelar','Clique neste botão para cancelar o envio '.($tab==0 ? 'da resposta ao despacho.' : 'd'.$config['genero_mensagem'].' '.$config['mensagem'].'.'),'','document.getElementById(\'tblTexto\').style.display=\'none\'; document.getElementById(\'tblDespachos\').style.display=\'\';').'</td></tr>';
echo '</table>';
...
for ($i = (($pagina - 1) * $xpg_tamanhoPagina); ($i < ($pagina * $xpg_tamanhoPagina)) && ($i < $xpg_totalregistros); $i++) {
	...
	if($tab==1 || $tab==3){
		...
		$sql->adOnde('msg_usuario.anotacao_id='.$linha['anotacao_id']);
		$destinatarios = $sql->Lista();
		$sql->limpar();
		$saida_destinatarios='';
	...
	$sql->adCampo('DISTINCT msg.msg_id, msg_usuario_id, referencia, texto');
	if ($tab==1 || $tab==3) $sql->adOnde('msg_usuario.de_id = '.$Aplic->usuario_id);
	if ($tab==0 || $tab==2) $sql->adOnde('msg_usuario.para_id = '.$Aplic->usuario_id);
	$sql->adOnde('tipo=1');
	if ($tab==0 || $tab==1)$sql->adOnde('resposta_despacho IS NULL AND data_limite IS NOT NULL');
	if ($tab==2 || $tab==3)$sql->adOnde('resposta_despacho IS NOT NULL');
	$sql->adOnde('msg_usuario.anotacao_id='.$linha['anotacao_id']);
	$sql->adGrupo('msg.msg_id, msg_usuario_id, referencia, texto');
	$mensagens = $sql->Lista();
	$sql->limpar();

	$saida_mensagens='';
	if ($mensagens && count($mensagens)) {
			$saida_mensagens.= '<a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a='.$Aplic->usuario_prefs['modelo_msg'].'&msg_usuario_id='.$mensagens[0]['msg_usuario_id'].'\');">'.dica($mensagens[0]['referencia'], $mensagens[0]['texto']).$mensagens[0]['msg_id'].' - '.$mensagens[0]['referencia'].dicaF();
			$qnt_mensagens=count($mensagens);
			if ($qnt_mensagens > 1) {
					$lista='';
					for ($j = 1, $j_cmp = $qnt_mensagens; $j < $j_cmp; $j++) $lista.='<a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a='.$Aplic->usuario_prefs['modelo_msg'].'&msg_usuario_id='.$mensagens[$j]['msg_id'].'\');">'.dica($mensagens[$j]['referencia'], $mensagens[$j]['texto']).$mensagens[$j]['msg_id'].' - '.$mensagens[$j]['referencia'].dicaF().'<br>';
					$saida_mensagens.= dica('Outr'.$config['genero_mensagem'].'s '.ucfirst($config['mensagens']), 'Clique para visualizar '.$config['genero_mensagem'].'s demais '.$config['mensagens'].'.').' <a href="javascript: void(0);" onclick="expandir_colapsar(\'mensagens_'.$linha['anotacao_id'].'\');">(+'.($qnt_mensagens - 1).')</a>'.dicaF(). '<span style="display: none" id="mensagens_'.$linha['anotacao_id'].'"><br>'.$lista.'</span>';
					}
			}
	...
	echo '<tr style="display: none" id="despacho_'.$linha['anotacao_id'].'"><td colspan=20 class="realce">'.$linha['texto'].'</td></tr>';
...
function editar_pastas(){
 	...
 	frmdespacho.retornar.value="lista_despacho";
	frmdespacho.submit();
  }
