...
if(isset($_REQUEST['carregarListas'])){
	...
	foreach ($resultado as $r) {
		$t['total']++;
		$t['list'][] = array('id'=> $r['id'], 'name' => htmlarray(gpw_passthrough($r['nome'])));
		}
	echo json_encode($t); 
	exit;
	}
elseif(isset($_REQUEST['carregarTarefas'])){
	...
	$s = trim(_get('s'));
	if($s != '') $sql->adOnde('(titulo LIKE '. $sql->quoteParecido("%%%s%%",previnirXSS(gpw_passthrough($s))).' OR nota LIKE '.$sql->quoteParecido("%%%s%%",previnirXSS(gpw_passthrough($s))).')');
	$sort = (int)_get('sort');
...
elseif(isset($_REQUEST['novaTarefa'])){
	...
	$sql->adInserir('titulo', previnirXSS(gpw_passthrough($titulo)));
	$sql->adInserir('d', $d);
	$sql->adInserir('ow', $ow);
	...
	if($parafazer_chave){
		...
		if($palavra_chave_ids) {
			...
			$sql->adTabela('parafazer_tarefa');
			$sql->adAtualizar('parafazer_chave', previnirXSS(gpw_passthrough($parafazer_chave)));
			$sql->adOnde('id= '.$id);
...
elseif(isset($_REQUEST['novaTarefaCompleta'])){
	...
	$sql->adInserir('ow', $ow);
	$sql->adInserir('prio', $prio);
	$sql->adInserir('nota', previnirXSS(gpw_passthrough($nota)));
...
elseif(isset($_REQUEST['editarNota'])){
	...
	$nota = str_replace("\r\n", "\n", trim(_post('nota')));
	$sql->adTabela('parafazer_tarefa');
	$sql->adAtualizar('nota', previnirXSS(gpw_passthrough($nota)));
	$sql->adOnde('id= '.$id);
	if (!$sql->exec()) die('Não foi possivel atualizar a tabela parafazer_tarefa!'.$bd->stderr(true));
	$sql->limpar();
	$t = array();
...
elseif(isset($_REQUEST['editarTarefa'])){
	...
	$sql->adTabela('parafazer_tarefa');
	$sql->adAtualizar('titulo', previnirXSS(gpw_passthrough($titulo)));
	$sql->adAtualizar('nota', previnirXSS(gpw_passthrough($nota)));
...
elseif(isset($_REQUEST['sugerirChaves'])){
	...
	$sql->adCampo('nome,id');
	$sql->adOnde('lista_id='. $listId);
	$sql->adOnde('nome LIKE \'%'.previnirXSS(gpw_passthrough($begin)).'%\'');
...
elseif(isset($_REQUEST['nuvemChave'])){
	...
	$resultados=$sql->Lista();
	$sql->limpar();
	foreach($resultados as $r) $a[gpw_passthrough($r['nome'])] = $r['cont_palavra_chave'];
	$t = array();
	$t['total'] = 0;
	$count = sizeof($a);
	if(!$count) {
...
elseif(isset($_REQUEST['renomearLista'])){
	...
	$nome = str_replace(array('"',"'",'<','>','&'),array('','','','',''),trim(_post('name')));
	$sql->adTabela('parafazer_listas');
	$sql->adAtualizar('nome', previnirXSS(gpw_passthrough($nome)));
	$sql->adOnde('id ='.$id);
	if (!$sql->exec()) die('Não foi possivel atualizar a tabela parafazer_listas!'.$bd->stderr(true));
	$sql->limpar();
...
function prepararLinhaTarefa($r, $tz=null){
	...
	return array(
		'id' => $r['id'],
		'titulo' => htmlarray(gpw_passthrough($r['titulo'])),
		'date' => htmlarray($r['d']),
		'compl' => (int)$r['compl'],
		'prio' => $r['prio'],
		'nota' => nl2br(htmlarray(gpw_passthrough($r['nota']))),
		'notaText' => (string)gpw_passthrough($r['nota']),
		'ow' => (int)$r['ow'],
		'parafazer_chave' => htmlarray(gpw_passthrough($r['parafazer_chave'])),
...
function pegar_ou_criar_chave($nome, $listId){
	...
	$q->adOnde('nome= \''.previnirXSS(gpw_passthrough($nome)).'\'');
	$tag = $q->Linha();
	$q->limpar();
	if($tag) return array($tag['id'], gpw_passthrough($tag['nome']));
	$q->adTabela('parafazer_chave');
	$q->adInserir('nome', previnirXSS(gpw_passthrough($nome)));
...
function get_palavra_chave_id($tag, $listId){
	...
	$q->adCampo('id');
	$q->adOnde('lista_id= '.$listId);
	$q->adOnde('nome=\''.previnirXSS(gpw_passthrough($tag)).'\'');
	$id = $q->Resultado();
	$q->limpar();
	return $id ? $id : 0;
	}
