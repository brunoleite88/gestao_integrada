...
echo '<th nowrap="nowrap">'.dica('Assunto', 'Assunto do fórum.').'Assunto'.dicaF().'</th><th>'.dica('Descrição', 'Descrição do fórum.').'Descrição'.dicaF().'</th><th nowrap="nowrap">'.dica('Número de '.ucfirst($config['mensagens']).'', 'Número de mensagens contídas neste fórum.').'Msg'.dicaF().'</th><th nowrap="nowrap">'.dica('Última Postagem', 'Última postagem de mensagem neste fórum.').'Última Postagem'.dicaF().'</th></tr>';
...
foreach ($rc as $linha)	{  
	...
	echo '<tr><td nowrap="nowrap" align="center" width="10">'.( $linha["forum_dono"] == $Aplic->usuario_id ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=foruns&a=editar&forum_id='.$linha['forum_id'].'\');"><img src="'.acharImagem('icones/editar.gif').'" alt="expandir forum" border=0 width=12 height=12></a>' : '').'</td>';
	...
	echo '<td nowrap="nowrap">'.$linha['forum_contagem_msg'].'</td>';
