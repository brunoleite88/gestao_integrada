...
function mudar_posicao_recurso($ordem, $recurso_tarefa_id, $direcao, $tarefa_id=0, $recurso_tarefa_uuid=''){
	...
	$objResposta->assign("lista_recursos","innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function editar_recurso($recurso_tarefa_id){
	...
	$objResposta->assign("recurso_id","value", gpw_passthrough($recurso_id));	
	return $objResposta;
	}	
...
function detalhes_recurso($recurso_id, $posicao){
	...
	if ($linha['recurso_tipo']==5){
		...
		$saida .='</table></td></tr>';
		if ($linha['recurso_tipo']==5 && $linha['recurso_nd']) $saida .= '<tr><td align="center" style="border: 1px solid;-webkit-border-radius:3.5px;"><b>ND</b></td><td>'.gpw_passthrough($linha['nd']).'</td></tr>';
		}
	else{
		if (isset($tipo[$linha['recurso_tipo']])) $saida .= '<tr><td align="center" style="border: 1px solid;-webkit-border-radius:3.5px;"><b>Tipo</b></td><td>'.gpw_passthrough($tipo[$linha['recurso_tipo']]).'</td></tr>';
		if ($linha['recurso_quantidade']) $saida .= '<tr><td align="center" style="border: 1px solid;-webkit-border-radius:3.5px;"><b>Total</b></td><td>'.gpw_passthrough(($linha['recurso_tipo']==5 ? $config['simbolo_moeda'].' '.number_format($linha['recurso_quantidade'], 2, ',', '.') : number_format($linha['recurso_quantidade'], 2, ',', '.'))).'</td></tr>';
		if (isset($TipoUnidade[$linha['recurso_unidade']])) $saida .= '<tr><td align="center" style="border: 1px solid;-webkit-border-radius:3.5px;"><b>Unidade</b></td><td>'.gpw_passthrough($TipoUnidade[$linha['recurso_unidade']]).'</td></tr>';
		}
	if ($linha['recurso_nota']) $saida .= '<tr><td align="center" style="border: 1px solid;-webkit-border-radius:3.5px;"><b>'.gpw_passthrough('Descrição').'</b></td><td>'.gpw_passthrough($linha['recurso_nota']).'</td></tr>';
	$saida .= '</table>';
	$objResposta = new xajaxResponse();
...
function ver_recursos_ajax($lista_cias='', $recurso_tipo=0, $recurso_responsavel=0, $recurso_ano='', $recurso_ugr='', $recurso_ptres='', $dept_id=0, $recurso_credito_adicional='', $recurso_movimentacao_orcamentaria='', $recurso_identificador_uso='', $recurso_pesquisa=''){
	...
	if ($recurso_pesquisa) $sql->adOnde('(recurso_nome LIKE \'%'.previnirXSS(gpw_passthrough($recurso_pesquisa)).'%\' OR recurso_chave LIKE \'%'.previnirXSS(gpw_passthrough($recurso_pesquisa)).'%\' OR recurso_nota LIKE \'%'.previnirXSS(gpw_passthrough($recurso_pesquisa)).'%\')');
	...
	foreach ($res as $linha) {
		if (permiteEditarRecurso($linha['recurso_nivel_acesso'], $linha['recurso_id'])) $todos_recursos[$linha['recurso_id']] = gpw_passthrough($linha['recurso_nome'].' ('.$recurso_tipos[$linha['recurso_tipo']].')');
		}
