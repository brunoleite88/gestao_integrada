...
if (!$dialogo && $Aplic->profissional){
	...
	$nova_avaliacao=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Nova Avaliação', 'Criar um nova avaliação.').'<a href="javascript: void(0)" onclick="javascript:frm_filtro.a.value=\'avaliacao_editar\'; frm_filtro.submit();" ><img src="'.acharImagem('avaliacao_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$vazio.'</table></td><td><table cellspacing=0 cellpadding=0>'.$nova_avaliacao.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	if ($podeAdicionar) $botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nova Avaliação', 'Criar um nova avaliação.').'<a class="botao" href="javascript: void(0)" onclick="javascript:frm_filtro.a.value=\'avaliacao_editar\'; frm_filtro.submit();" ><span>nova&nbsp;avaliação</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
