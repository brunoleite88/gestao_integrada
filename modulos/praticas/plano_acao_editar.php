...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Progresso', 'O plan '.$config['genero_acao'].' '.$config['acao'].' pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td nowrap="nowrap">'.selecionaVetor($percentual, 'plano_acao_percentagem', 'size="1" class="texto"'.($Aplic->profissional && $exibir['porcentagem_item'] && $obj->plano_acao_calculo_porcentagem ? ' disabled' : ''), (int)$obj->plano_acao_percentagem).'% </td></tr>';
