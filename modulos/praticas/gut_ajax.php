...
//	$xajax->setFlag('outputEntities',true);
	...
	function inserir_linha_ajax($gut_id=0, $gut_texto='', $gut_g=0, $gut_t=0, $gut_u=0, $gut_linha_id=0){
		...
		if($gut_linha_id){
			...
			$sql->adAtualizar('gut_texto', previnirXSS(gpw_passthrough($gut_texto)));
			$sql->adOnde('gut_linha_id='.(int)$gut_linha_id);
			$sql->exec();
			$sql->limpar();
			}
			...
			$sql->adInserir('gut_texto', previnirXSS(gpw_passthrough($gut_texto)));
			$sql->exec();
			$sql->limpar();
			}
...
function mudar_campos_ajax($cia_id=0){
	...
	foreach($linhas as $linha) $lista_projetos[$linha['projeto_id']]=gpw_passthrough($linha['projeto_nome']);
	...
	foreach($linhas as $linha) $lista_indicadores[$linha['pratica_indicador_id']]=gpw_passthrough($linha['pratica_indicador_nome']);
	...
	foreach($linhas as $linha) $lista_praticas[$linha['pratica_id']]=gpw_passthrough($linha['pratica_nome']);
