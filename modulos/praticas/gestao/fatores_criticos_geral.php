...
$pg_fator_critico_nome=getParam($_REQUEST, 'pg_fator_critico_nome', '0');
...
$mudar_pg_fator_critico_id=getParam($_REQUEST, 'mudar_pg_fator_critico_id', '0');
...
echo '<input type="hidden" name="mudar_pg_fator_critico_id" value="" />';
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\''.($Aplic->profissional && $config['exibe_me']  && $Aplic->checarModulo('praticas', 'acesso', null, 'me') ? 'mes_pro' : 'objetivos_estrategicos').'\');').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'fatores_criticos\');').'</td></tr></table></td></tr>';
