...
if ($acao!='inserir' && $acao!='editar'){
	...
	foreach ($linhas as $linha) {
		...
		echo '<td align="right">'.number_format($linha['plano_acao_item_custos_custo'], 2, ',', '.').'</td>';
		...
		echo '</tr>';
		if (isset($custo[$linha['plano_acao_item_custos_nd']])) $custo[$linha['plano_acao_item_custos_nd']] += (float)$linha['valor'];
		else $custo[$linha['plano_acao_item_custos_nd']] =(float)$linha['valor'];
		$total+=$linha['valor'];
		}
	if ($total) {
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td>'.$config['simbolo_moeda'].'&nbsp;<input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="plano_acao_item_custos_custo" id="plano_acao_item_custos_custo" value="'.(isset($atual['plano_acao_item_custos_custo']) ? number_format($atual['plano_acao_item_custos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
	...
	echo '<tr><td align="right">'.dica('Data','Data limite para o recebimento do ítem.').'Data:</td><td><table cellpadding=0 cellspacing=0><tr><td><td><input type="hidden" name="plano_acao_item_custos_data_limite" id="plano_acao_item_custos_data_limite" value="'.($data_texto ? $data_texto->format(FMT_TIMESTAMP_DATA) : '').'" /><input type="text" name="data_texto"  id="data_texto" style="width:70px;" onchange="setData(\'env\', \'data_texto\', \'plano_acao_item_custos_data_limite\');" value="'.($data_texto ? $data_texto->format($df) : '').'" class="texto" />'.dica('Data Limite', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data limite para o recebimento do ítem.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr></table></td></tr>';
