...
function exibir_usuarios($usuarios){
	...
	$objResposta->assign('combo_usuarios',"innerHTML", gpw_passthrough($saida_usuarios));
	return $objResposta;				
	}
...
function exibir_depts($depts){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign('combo_depts',"innerHTML", gpw_passthrough($saida_depts));
	return $objResposta;				
	}
...
function editar_filtro($pratica_indicador_filtro_id=null){
	...
	$objResposta->assign("pratica_indicador_filtro_status","value", ($linha['pratica_indicador_filtro_status'] ? gpw_passthrough($linha['pratica_indicador_filtro_status']) : ''));
	$tarefa_tipos=vetor_campo_sistema('TipoTarefa', $linha['pratica_indicador_filtro_tipo']);
	$objResposta->assign("combo_tarefa_tipo","innerHTML", gpw_passthrough(selecionaVetor($tarefa_tipos, 'pratica_indicador_filtro_tipo', 'class="texto" size=1 style="width:284px;" onchange="mudar_tarefa_tipo();"', $linha['pratica_indicador_filtro_tipo'])));
	$objResposta->assign("pratica_indicador_filtro_prioridade","value", ($linha['pratica_indicador_filtro_prioridade'] ? gpw_passthrough($linha['pratica_indicador_filtro_prioridade']) : ''));
	$objResposta->assign("pratica_indicador_filtro_setor","value",  ($linha['pratica_indicador_filtro_setor'] ? gpw_passthrough($linha['pratica_indicador_filtro_setor']) : ''));
	...
	$objResposta->assign("combo_segmento_tarefa","innerHTML", gpw_passthrough(selecionaVetor($segmento, 'pratica_indicador_filtro_segmento', 'style="width:284px;" class="texto" onchange="mudar_intervencao_tarefa();"', $linha['pratica_indicador_filtro_segmento'])));
	...
	$objResposta->assign("combo_intervencao_tarefa","innerHTML", gpw_passthrough(selecionaVetor($intervencao, 'pratica_indicador_filtro_intervencao', 'style="width:284px;" class="texto" onchange="mudar_tipo_intervencao_tarefa();"', $linha['pratica_indicador_filtro_intervencao'])));
	...
	$objResposta->assign("combo_tipo_intervencao_tarefa","innerHTML", gpw_passthrough(selecionaVetor($tipo_intervencao, 'pratica_indicador_filtro_tipo_intervencao', 'style="width:284px;" class="texto"', $linha['pratica_indicador_filtro_tipo_intervencao'])));

	if ($social) {
		$objResposta->assign("pratica_indicador_filtro_social","value", ($linha['pratica_indicador_filtro_social'] ? gpw_passthrough($linha['pratica_indicador_filtro_social']) : ''));
		require_once BASE_DIR.'/modulos/social/social.class.php';
		$objResposta->assign("acao_combo_tarefa","innerHTML", gpw_passthrough(selecionar_acao_para_ajax($linha['pratica_indicador_filtro_social'], 'pratica_indicador_filtro_acao', 'size="1" style="width:284px;" class="texto"', '', $linha['pratica_indicador_filtro_acao'], false)));
		}

	$objResposta->assign("pratica_indicador_filtro_estado","value", ($linha['pratica_indicador_filtro_estado'] ? gpw_passthrough($linha['pratica_indicador_filtro_estado']) : ''));
	$objResposta->assign("combo_cidade_tarefa","innerHTML", gpw_passthrough(selecionar_cidades_para_ajax($linha['pratica_indicador_filtro_estado'], 'pratica_indicador_filtro_cidade', 'class="texto" '.($social ? 'onchange="mudar_comunidades_tarefa()"' : '').' style="width:284px;"', '', $linha['pratica_indicador_filtro_cidade'], true, false)));
	if ($social) $objResposta->assign("combo_comunidade_tarefa","innerHTML", gpw_passthrough(selecionar_comunidade_para_ajax($linha['pratica_indicador_filtro_cidade'],'pratica_indicador_filtro_comunidade', 'class="texto" style="width:284px;"', '', $linha['pratica_indicador_filtro_comunidade'], false)));
	$objResposta->assign("pratica_indicador_filtro_texto","value", ($linha['pratica_indicador_filtro_texto'] ? gpw_passthrough($linha['pratica_indicador_filtro_texto']) : ''));
...
function excluir_filtro($pratica_indicador_filtro_id, $pratica_indicador_id=null, $uuid=null){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign('combo_filtros',"innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function incluir_filtro(
	...
	if (!$filtro_id){
		...
		if ($filtro_status) $sql->adInserir('pratica_indicador_filtro_status', previnirXSS(gpw_passthrough($filtro_status)));
		if ($prioridade) $sql->adInserir('pratica_indicador_filtro_prioridade', previnirXSS(gpw_passthrough($prioridade)));
		if ($tipo) $sql->adInserir('pratica_indicador_filtro_tipo', previnirXSS(gpw_passthrough($tipo)));
		if ($setor) $sql->adInserir('pratica_indicador_filtro_setor', previnirXSS(gpw_passthrough($setor)));
		if ($segmento) $sql->adInserir('pratica_indicador_filtro_segmento', previnirXSS(gpw_passthrough($segmento)));
		if ($intervencao) $sql->adInserir('pratica_indicador_filtro_intervencao', previnirXSS(gpw_passthrough($intervencao)));
		if ($tipo_intervencao) $sql->adInserir('pratica_indicador_filtro_tipo_intervencao', previnirXSS(gpw_passthrough($tipo_intervencao)));
		if ($social) $sql->adInserir('pratica_indicador_filtro_social', previnirXSS(gpw_passthrough($social)));
		if ($acao) $sql->adInserir('pratica_indicador_filtro_acao', previnirXSS(gpw_passthrough($acao)));
		if ($estado) $sql->adInserir('pratica_indicador_filtro_estado', previnirXSS(gpw_passthrough($estado)));
		if ($cidade) $sql->adInserir('pratica_indicador_filtro_cidade', previnirXSS(gpw_passthrough($cidade)));
		if ($comunidade) $sql->adInserir('pratica_indicador_filtro_comunidade', previnirXSS(gpw_passthrough($comunidade)));
		if ($texto) $sql->adInserir('pratica_indicador_filtro_texto', previnirXSS(gpw_passthrough($texto)));
		$sql->exec();
		$sql->limpar();
		}
	else{
		$sql->adAtualizar('pratica_indicador_filtro_status', previnirXSS(gpw_passthrough(($filtro_status ? $filtro_status : null))));
		$sql->adAtualizar('pratica_indicador_filtro_prioridade', previnirXSS(gpw_passthrough(($prioridade ? $prioridade : null))));
		$sql->adAtualizar('pratica_indicador_filtro_tipo', previnirXSS(gpw_passthrough(($tipo ? $tipo : null))));
		$sql->adAtualizar('pratica_indicador_filtro_setor', previnirXSS(gpw_passthrough(($setor ? $setor : null))));
		$sql->adAtualizar('pratica_indicador_filtro_segmento', previnirXSS(gpw_passthrough(($segmento ? $segmento : null))));
		$sql->adAtualizar('pratica_indicador_filtro_intervencao', previnirXSS(gpw_passthrough(($intervencao ? $intervencao : null))));
		$sql->adAtualizar('pratica_indicador_filtro_tipo_intervencao', previnirXSS(gpw_passthrough(($tipo_intervencao ? $tipo_intervencao : null))));
		$sql->adAtualizar('pratica_indicador_filtro_social', previnirXSS(gpw_passthrough(($social ? $social : null))));
		$sql->adAtualizar('pratica_indicador_filtro_acao', previnirXSS(gpw_passthrough(($acao ? $acao : null))));
		$sql->adAtualizar('pratica_indicador_filtro_estado', previnirXSS(gpw_passthrough(($estado ? $estado : null))));
		$sql->adAtualizar('pratica_indicador_filtro_cidade', previnirXSS(gpw_passthrough(($cidade ? $cidade : null))));
		$sql->adAtualizar('pratica_indicador_filtro_comunidade', previnirXSS(gpw_passthrough(($comunidade ? $comunidade : null))));
		$sql->adAtualizar('pratica_indicador_filtro_texto', previnirXSS(gpw_passthrough(($texto ? $texto : null))));
...
function mudar_ajax($superior='', $sisvalor_titulo='', $campo='', $posicao, $script){
	...
	$sql->limpar();
	$vetor=array(0 => '&nbsp;');	
	foreach($lista as $linha) $vetor[gpw_passthrough($linha['sisvalor_valor_id'])]=gpw_passthrough($linha['sisvalor_valor']);	
...
function excluir_meta($pratica_indicador_meta_id=null, $pratica_indicador_meta_indicador=null, $uuid=null){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign("metas","innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function mudar_pauta($pratica_indicador_id=0, $uuid='', $pratica_modelo_id=0, $ano=''){
	...
	foreach($lista as $linha){	
		if (in_array($linha['pratica_regra_campo_nome'], $vetor_existe)){
			$campo=gpw_passthrough(dica($linha['pratica_regra_campo_texto'], $linha['pratica_regra_campo_descricao']).'<b>'.$linha['pratica_regra_campo_texto'].'</b>:'.dicaF());
	...
	foreach($original as $chave => $valor) $objResposta->assign('legenda_'.$chave,"innerHTML", gpw_passthrough($valor));
	return $objResposta;
	}
...
function mudar_usuario_ajax($cia_id=0, $usuario_id=0, $campo='', $posicao='', $script='', $segunda_tabela='', $condicao=''){
	...
	foreach((array)$linhas as $linha) {
		$vetor[$linha['usuario_id']]=gpw_passthrough($linha['nome_usuario']);
		}
