...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo = new CBlocoTitulo(ucfirst($config['projetos']).' por '.ucfirst($config['objetivos']), 'mapa_estrategico.gif', $m, $m.'.'.$a);
	$botoesTitulo->adicionaCelula(dica('Plano de Gestão', 'Utilize esta opção para filtrar de qual '.$config['plano_gestao'].' será visualidado o mapa estratégico.').'Plano de Gestão:'.dicaF().selecionaVetor($planos, 'pg_id', 'onchange="document.frm_filtro.submit()" class="texto"', $ano));
...
foreach($perspectivas as $perspectiva){
	...
	foreach($objetivos as $objetivo){
		...
		if ($Aplic->profissional) $sql->adOnde('projeto_gestao_objetivo='.(int)$objetivo['pg_objetivo_estrategico_id']);
		else $sql->adOnde('projeto_objetivo_estrategico='.(int)$objetivo['pg_objetivo_estrategico_id']);
		$projetos=$sql->carregarColuna();
		$sql->limpar();
		$saida='';
		foreach($projetos as $projeto) $saida.=($qnt++ ? '<br>' : '').link_projeto($projeto);
		echo '<td align=center><a href="javascript: void(0);" onclick="expandir_colapsar(\'lista_'.$objetivo['pg_objetivo_estrategico_id'].'\');">'.count($projetos).'</a>'.dicaF(). '<span style="display: none" id="lista_'.$objetivo['pg_objetivo_estrategico_id'].'"><br>'.$saida.'</span></td>';
		...
		if ($Aplic->profissional) $sql->adOnde('pratica_indicador_gestao_objetivo='.(int)$objetivo['pg_objetivo_estrategico_id']);
		else $sql->adOnde('pratica_indicador_objetivo_estrategico='.(int)$objetivo['pg_objetivo_estrategico_id']);
		$indicadores=$sql->carregarColuna();
		$sql->limpar();
		$saida='';
		foreach($indicadores as $indicador) $saida.=($qnt++ ? '<br>' : '').link_indicador($indicador);
		echo '<td align=center><a href="javascript: void(0);" onclick="expandir_colapsar(\'indicador_'.$objetivo['pg_objetivo_estrategico_id'].'\');">'.count($indicadores).'</a>'.dicaF(). '<span style="display: none" id="indicador_'.$objetivo['pg_objetivo_estrategico_id'].'"><br>'.$saida.'</span></td>';
		echo '</tr>';
		}
