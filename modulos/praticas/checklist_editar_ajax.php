...
function mudar_posicao_gestao($ordem, $checklist_gestao_id, $direcao, $checklist_id=0, $uuid=''){
	...
	$objResposta = new xajaxResponse();
	$objResposta->assign("combo_gestao","innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function incluir_pergunta_ajax($checklist_id=null, $uuid='', $checklist_lista_id, $checklist_lista_peso='', $checklist_lista_descricao='', $checklist_lista_legenda=false){
	...
	$checklist_lista_peso=previnirXSS(gpw_passthrough($checklist_lista_peso));
	$checklist_lista_descricao=previnirXSS(gpw_passthrough($checklist_lista_descricao));
...
function atualizar_perguntas($checklist_id=null, $uuid=''){
	...
	if (count($perguntas)) {
		...
		foreach ($perguntas as $checklist_lista_id => $linha) {
			...
			$saida.= '<td align="left" '.($linha['checklist_lista_legenda'] ? 'colspan=2' : '').'>'.gpw_passthrough($linha['checklist_lista_descricao']).'</td>';
...
function editar_pergunta($checklist_lista_id){
	...
	$objResposta->assign("texto_apoio","value", gpw_passthrough($linha['checklist_lista_descricao']));	
