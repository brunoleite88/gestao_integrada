...
$favorito_id = $Aplic->getEstado('meta_favorito') !== null ? $Aplic->getEstado('estrategia_favorito') : 0;
...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.'</table></td><td><table cellspacing=0 cellpadding=0>'.$nova_meta.$imprimir.$botao_favorito.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	//$botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de metas favoritas, para uma rápida filtragem.').'<a class="botao" href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&meta=1\');"><span>favoritos</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap">'.$favoritos.'</td></tr></table>');
	if ($podeAdicionar) $botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Criar nov'.$config['genero_meta'].' '.$config['meta'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:frm_filtro.a.value=\'meta_editar\'; frm_filtro.submit();" ><span>nova</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['metas']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['metas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=meta_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
