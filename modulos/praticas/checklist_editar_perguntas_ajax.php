...
function atualizar_perguntas($checklist_lista_checklist_id){
	...
	if (count($perguntas)) {
		...
		foreach ($perguntas as $checklist_lista_id => $linha) {
			...
			$saida.= '</td>';
			if (!$linha['checklist_lista_legenda']) $saida.= '<td align="left" nowrap="nowrap">'.gpw_passthrough((int)$linha['checklist_lista_peso']).'</td>';
...
function editar_pergunta($checklist_lista_id){
	...
	$objResposta->assign("checklist_lista_peso","value", gpw_passthrough((int)$linha['checklist_lista_peso']));
