...
gpweb\modulos\praticas\causa_efeito_js.php		

Funções javascript utilizadas em gpweb\modulos\praticas\causa_efeito.php																																							
...
function popProjeto() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["projeto"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto&edicao=1&aceita_portfolio=1&tabela=projetos&cia_id='+document.getElementById('causa_efeito_cia').value, window.setProjeto, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto&aceita_portfolio=1&tabela=projetos&cia_id='+document.getElementById('causa_efeito_cia').value, 'Projetos','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}

function popTarefa() {
	if (!codigo.projetos_escolhidos.value) alert('Necessário selecionar projeto primeiro');
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setTarefa&tabela=tarefas&projeto_id='+document.getElementById('projetos_escolhidos').value+'&cia_id='+document.getElementById('causa_efeito_cia').value, 'Tarefas','left=0,top=0,height=600,width=350,scrollbars=yes, resizable=yes');
	}
function popPratica() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["pratica"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setPratica&tabela=praticas&cia_id='+document.getElementById('causa_efeito_cia').value, window.setPratica, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setPratica&tabela=praticas&cia_id='+document.getElementById('causa_efeito_cia').value, 'Prática','left=0,top=0,height=600,width=350,scrollbars=yes, resizable=yes');
	}	
function popIndicador() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Indicador', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setIndicador&tabela=pratica_indicador&cia_id='+document.getElementById('causa_efeito_cia').value, window.setIndicador, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setIndicador&tabela=pratica_indicador&cia_id='+document.getElementById('causa_efeito_cia').value, 'Indicador','left=0,top=0,height=600,width=350,scrollbars=yes, resizable=yes');
	}
...
function popIndicador_filtro() {
	...
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setIndicador_filtro&tabela=pratica_indicador&cia_id='+document.getElementById('cia_id').value, 'Indicadores','left=0,top=0,height=600,width=350,scrollbars=yes, resizable=yes');
	}
...
function setUsuarios(usuario_id_string){
	...
	document.getElementById('causa_efeito_usuarios').value = usuario_id_string;
...
function setDepts(departamento_id_string){
	...
	document.getElementById('causa_efeito_depts').value = departamento_id_string;
	depts_id_selecionados = departamento_id_string;
	}

function mudar_dept(){
	xajax_exibir_dept('causa_efeito_dept', 'dept_cia='+document.getElementById('causa_efeito_cia').value, 'style=\'width:200px;\' size=\'1\' class=\'texto\'', 'causa_efeito_dept', '', true);
	}

function mudar_projeto(){
	xajax_exibir_combo('causa_efeito_projeto', 'projetos',  'projeto_id', 'projeto_nome', 'projeto_cia='+document.getElementById('causa_efeito_cia').value, '', 'onchange=\'mudar_tarefa();\' style=\'width:250px;\' size=\'3\' class=\'texto\' ondblclick=\'Mover(document.codigo.lista_projetos, document.codigo.projetos_escolhidos);\'', 'lista_projetos', '', true,'','','','');
	}


function mudar_tarefa(){
	xajax_exibir_combo('causa_efeito_tarefa', 'tarefas',  'tarefa_id', 'tarefa_nome', 'tarefa_projeto='+document.getElementById('lista_projetos').value, 'tarefa_inicio', 'style=\'width:250px;\' size=\'3\' class=\'texto\' ondblclick=\'Mover(document.codigo.lista_tarefas, document.codigo.tarefas_escolhidas);\'', 'lista_tarefas', '', true,'','','','');
	}


function mudar_indicador(){
	xajax_exibir_combo('causa_efeito_indicador', 'pratica_indicador',  'pratica_indicador_id', 'pratica_indicador_nome', 'pratica_indicador_cia='+document.getElementById('causa_efeito_cia').value, '', 'style=\'width:250px;\' size=\'3\' class=\'texto\' ondblclick=\'Mover(document.codigo.lista_indicadores, document.codigo.indicadores_escolhidos);\'', 'lista_indicadores', '', true,'','','','');
	}

function mudar_pratica(){
	xajax_exibir_combo('causa_efeito_pratica', 'praticas',  'pratica_id', 'pratica_nome', 'pratica_cia='+document.getElementById('causa_efeito_cia').value, '', 'style=\'width:250px;\' size=\'3\' class=\'texto\' ondblclick=\'Mover(document.codigo.lista_praticas, document.codigo.praticas_escolhidas);\'', 'lista_praticas', '', true,'','','','');
	}
...
function salvar_causa_efeito(){
	...
	if(!document.codigo.causa_efeito_nome.value)alert('O diagrama necessita ter um nome!');
	else {
