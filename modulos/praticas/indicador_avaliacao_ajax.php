...
function mudar_posicao_causa($pratica_indicador_avaliacao_ordem, $pratica_indicador_avaliacao_id, $direcao, $pratica_indicador_id=null){
	...
	$objResposta->assign("combo_causa","innerHTML", gpw_passthrough($saida));
	return $objResposta;
	}
...
function incluir_causa(
	...
	$sql = new BDConsulta;
	$pratica_indicador_avaliacao_causa=previnirXSS(gpw_passthrough($pratica_indicador_avaliacao_causa));
	$pratica_indicador_avaliacao_sanar=previnirXSS(gpw_passthrough($pratica_indicador_avaliacao_sanar));
...
function editar_causa($pratica_indicador_avaliacao_id){
	...
	if ($linha['pratica_indicador_avaliacao_sucesso']){
		$objResposta->assign("sucesso","value", gpw_passthrough($linha['pratica_indicador_avaliacao_causa']));
		$objResposta->assign("insucesso","value", '');
		}
	else {
		$objResposta->assign("insucesso","value", gpw_passthrough($linha['pratica_indicador_avaliacao_causa']));
		$objResposta->assign("sucesso","value", '');
		}
	
	$objResposta->assign("pratica_indicador_avaliacao_sanar","value", gpw_passthrough($linha['pratica_indicador_avaliacao_sanar']));
	$objResposta->assign("apoio1","value", gpw_passthrough($linha['pratica_indicador_avaliacao_sanar']));
