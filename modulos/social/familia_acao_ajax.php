...
function incluir_problema_ajax($acao_id=0,$familia=0, $problema=0, $observacao=''){
	...
	$sql->adInserir('social_familia_problema_observacao', previnirXSS(gpw_passthrough(gpw_passthrough($observacao))));
...
function exibir_problema_ajax($acao_id=0, $familia=0){
	...
	foreach ($lista as $linha) {
		$saida.='<tr>';
		$saida.='<td>'.(isset($lista_problemas[$linha['social_familia_problema_tipo']]) ? gpw_passthrough($lista_problemas[$linha['social_familia_problema_tipo']]) : '&nbsp;').'</td>';
		$saida.='<td>'.($linha['social_familia_problema_observacao'] ? gpw_passthrough($linha['social_familia_problema_observacao']) : '&nbsp;').'</td>';
		$saida.='<td>'.retorna_data($linha['social_familia_problema_data_insercao'], false).'</td>';
		$saida.='<td>'.($linha['social_familia_problema_usuario_insercao'] ? gpw_passthrough(link_usuario($linha['social_familia_problema_usuario_insercao'], '','','esquerda','','',false)): gpw_passthrough($linha['social_familia_problema_usuario_insercao_nome'])).'</td>';
		$saida.='<td>'.(isset($status[$linha['social_familia_problema_status']]) ? gpw_passthrough($status[$linha['social_familia_problema_status']]) : '&nbsp;').'</td>';
	...
	if ($saida) $saida='<table cellpadding=0 cellspacing=0 class="tbl1"><tr><th>Problema</th><th>'.gpw_passthrough('Observação').'</th><th>Data</th><th>'.gpw_passthrough('Responsável').'</th><th>Status</th><th></th></tr>'.$saida.'</table>';
