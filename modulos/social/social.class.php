...
function selecionar_comunidade_para_ajax($social_comunidade_municipio='', $campo='', $script='', $vazio='', $social_comunidade_id=0, $ajax=true){
	...
	$vetor=array();
	$vetor['']=$vazio;
	if ($ajax) foreach($comunidades as $linha) $vetor[gpw_passthrough($linha['social_comunidade_id'])]=gpw_passthrough($linha['social_comunidade_nome']);
...
function selecionar_acao_para_ajax($social_id=0, $campo='', $script='size="1" style="width:160px;" class="texto"', $vazio='', $acao_id=0, $ajax=true){
	...
	$lista=$sql->Lista();
	$sql->limpar();
	if ($ajax) foreach ($lista as $linha) $lista_acoes[$linha['social_acao_id']]=gpw_passthrough($linha['social_acao_nome']);
...
function selecionar_problema_para_ajax($acao_id=0, $campo='', $script='size="1" style="width:160px;" class="texto"', $vazio='', $problema_id=0, $ajax=true, $tipo=0){
	...
	$lista=$sql->Lista();
	$sql->limpar();
	if ($ajax) foreach ($lista as $linha) $lista_problemas[$linha['social_acao_problema_id']]=gpw_passthrough($linha['social_acao_problema_descricao']);
...
function selecionar_acao_negacao_para_ajax($acao_id=0, $campo='', $script='size="1" style="width:160px;" class="texto"', $vazio='', $negativa_id=0, $ajax=true){
	...
	$lista=$sql->Lista();
	$sql->limpar();
	if ($ajax) foreach ($lista as $linha) $lista_acoes[$linha['social_acao_negacao_id']]=gpw_passthrough($linha['social_acao_negacao_justificativa']);
