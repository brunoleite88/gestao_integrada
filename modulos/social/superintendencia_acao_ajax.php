...
function incluir_problema_ajax($acao_id=0,$familia=0, $problema=0, $observacao=''){
	...
	$sql->adInserir('social_superintendencia_problema_observacao', previnirXSS(gpw_passthrough($observacao)));
...
function exibir_problema_ajax($acao_id=0, $social_superintendencia_id=0){
	...
	foreach ($lista as $linha) {
		$saida.='<tr>';
		$saida.='<td>'.(isset($lista_problemas[$linha['social_superintendencia_problema_tipo']]) ? gpw_passthrough($lista_problemas[$linha['social_superintendencia_problema_tipo']]) : '&nbsp;').'</td>';
		$saida.='<td>'.($linha['social_superintendencia_problema_observacao'] ? gpw_passthrough($linha['social_superintendencia_problema_observacao']) : '&nbsp;').'</td>';
		$saida.='<td>'.retorna_data($linha['social_superintendencia_problema_data_insercao'], false).'</td>';
		$saida.='<td>'.gpw_passthrough(link_usuario($linha['social_superintendencia_problema_usuario_insercao'], '','','esquerda','','',false)).'</td>';
		$saida.='<td>'.(isset($status[$linha['social_superintendencia_problema_status']]) ? gpw_passthrough($status[$linha['social_superintendencia_problema_status']]) : '&nbsp;').'</td>';
