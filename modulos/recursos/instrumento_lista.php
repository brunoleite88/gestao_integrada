...
if (isset($_REQUEST['instrumento_ano']) || $filtro_acionado) $Aplic->setEstado('instrumento_ano', getParam($_REQUEST, 'instrumento_ano', null));
$instrumento_ano = ($Aplic->getEstado('instrumento_ano') ? $Aplic->getEstado('instrumento_ano') : ($filtro_acionado ? null : -1));

if (isset($_REQUEST['instrumento_tipo']) || $filtro_acionado) $Aplic->setEstado('instrumento_tipo', getParam($_REQUEST, 'instrumento_tipo', null));
$instrumento_tipo = ($Aplic->getEstado('instrumento_tipo') ? $Aplic->getEstado('instrumento_tipo') : ($filtro_acionado ? null : -1));

if (isset($_REQUEST['instrumento_situacao']) || $filtro_acionado) $Aplic->setEstado('instrumento_situacao', getParam($_REQUEST, 'instrumento_situacao', null));
$instrumento_situacao = ($Aplic->getEstado('instrumento_situacao') ? $Aplic->getEstado('instrumento_situacao') : ($filtro_acionado ? null : -1));
...
if (!$dialogo){
	...
	elseif($template_id){
		...
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar os instrumentos.').'Filtro:'.dicaF();
		}
	
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionado','A qual parte do sistema os instrumentos jurídicos estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional){	
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Instrumentos', 'instrumento.png', $m, "$m.$a");
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de instrumentos.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=recursos&a=instrumento_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';


	$novo=($podeEditar ? '<tr><td nowrap="nowrap">'.dica('Novo Instrumento', 'Criar um novo instrumento.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=recursos&a=instrumento_editar\');" ><img src="'.acharImagem('instrumento_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' :'');

	$botao_excel='<tr><td align=right><a href="javascript: void(0)" onclick="exportar_excel();">'.imagem('icones/excel_p.gif', 'Exportar para Excel' , 'Clique neste ícone '.imagem('icones/excel_p.gif').' para exportar a lista de instrumentos para o formato excel.').'</a>'.dicaF().'</td></tr>';
	...
	$procurar_tipo='<tr><td align="right" nowrap="nowrap">'.dica('Tipo', 'Selecione qual o tipo de instrumento.').'Tipo:'.dicaF().'</td><td align="left" >'.selecionaVetor(getSisValor('TipoInstrumento'), 'instrumento_tipo', 'class="texto" style="width:250px" multiple', $instrumento_tipo).'</td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procuraBuffer.$procurar_usuario.$procurar_ano.$procurar_tipo.$procurar_situacao.$popFiltro.$filtros.'</table></td><td><table cellspacing=0 cellpadding=0>'.$botao_filtrar.$novo.$imprimir.$botao_excel.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	if ($podeEditar)$botoesTitulo->adicionaCelula('<table width=75><tr><td nowrap="nowrap">'.dica('Novo Instrumento', 'Criar um novo instrumento.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=recursos&a=instrumento_editar\');" ><span>novo</span></a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->mostrar();
	}
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/recursos/instrumento_lista_idx', 'Ativos',null,null,'Ativos','Visualizar os instrumentos ativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/recursos/instrumento_lista_idx', 'Inativos',null,null,'Inativos','Visualizar os instrumentos inativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/recursos/instrumento_lista_idx', 'Todos',null,null,'Todos','Visualizar todos os instrumentos.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/recursos/instrumento_lista_idx.php');
...
function exportar_excel(){
  url_passar(1, 'm=recursos&a=instrumentos_exportar_excel_pro&sem_cabecalho=1');
	}
