...
class ImportarWBS extends CImportar {
  ...
  public function visualizar() {
    ...
    $data=gpw_passthrough($data);
    $arquivo_xml = simplexml_load_string($data);
    $projeto_nome = gpw_passthrough($arquivo_xml->proj->summary['Title']);
	...
	$qnt=(is_array($trabalhadores) ? count($trabalhadores) : 0);	
	    ...
	    foreach($trabalhadores as $r) {
				$nome=previnirXSS(gpw_passthrough($r['name']));
    ...
    $saida .= '<tr><td colspan="2">Tarefas:</td></tr>';
      ...
      if ($tarefa['ID'] != 0) {
        ...
        $saida .= '<input type="hidden" name="tasks['.$tarefa['ID'].'][task_name]" value="'.gpw_passthrough($tarefa['Name']).'" />';
        $saida .= '<input type="hidden" name="tasks['.$tarefa['ID'].'][task_description]" value="'.gpw_passthrough($note).'" />';
        $prioridade = 0;
        ...
        $saida .= gpw_passthrough($tarefa['Name']).($tarefa['NOTES']? '<br /><hr />'.gpw_passthrough($tarefa['NOTES']).'<hr size="2" />' : '') .'</td><td align="center">'.substr($tarefa['Start'],0,10).'</td><td align="center">'.substr($tarefa['Finish'],0,10).'</td><td>';
        $recurso='';
        $perc = 100;
