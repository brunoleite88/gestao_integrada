...
class ImportarMSProject extends CImportar {
  ...
  public function visualizar() {
    ...
    $data = $this->scrubbedData;
		$data = ($this->formatacao=='utf' ? iconv("UTF-8","UTF-8//IGNORE",$data) : iconv("utf-8","utf-8//IGNORE",$data));
    $arquivo_xml = simplexml_load_string($data);
    $projeto_nome =($this->formatacao=='utf' ? gpw_passthrough($arquivo_xml->Title) : $arquivo_xml->Title);
    ...
    $tipo_material = array();
   	foreach($arquivo_xml->Resources->children() as $r) {
    	if ($r->Name && $r->Type==1){ 	
    		//é uma pessoa
        $nome=($this->formatacao=='utf' ? gpw_passthrough($r->Name) : $r->Name);
    ...
    foreach($arquivo_xml->Tasks->children() as $tarefa) {	
      if ((int)$tarefa->UID>0) {
        ...
        $saida .= '<input type="hidden" name="tasks['.$k.'][task_name]" value="'.($this->formatacao=='utf' ? gpw_passthrough($tarefa->Name) : $tarefa->Name).'" />';
        $saida .= '<input type="hidden" name="tasks['.$k.'][task_description]" value="'.$this->tira_quebra((isset($tarefa->Notes) ? ($this->formatacao=='utf' ? gpw_passthrough($tarefa->Notes) : $tarefa->Notes) : '')).'" />';
        ...
        $saida .= ($this->formatacao=='utf' ? gpw_passthrough($tarefa->Name) : $tarefa->Name);
        if ($marco) $saida .= imagem('icones/marco.gif');
        if (!empty($tarefa->Notes)) $saida .= '<br /><hr />'.$this->tira_quebra(($this->formatacao=='utf' ? gpw_passthrough($tarefa->Notes) : $tarefa->Notes)).'<hr size="2" />';
