...
if (is_array($tarefas)) {
	foreach ($tarefas as $t) {
		if ($localidade_tipo_caract == 'utf-8' && function_exists('utf8_decode')) $nome = strlen(gpw_passthrough($t['tarefa_nome'])) > 25 ? substr(gpw_passthrough($t['tarefa_nome']), 0, 22).'...' : gpw_passthrough($t['tarefa_nome']);
		else $nome = strlen($t['tarefa_nome']) > 30 ? substr($t['tarefa_nome'], 0, 28).'...' : $t['tarefa_nome'];
		
		if ($localidade_tipo_caract == 'utf-8' && function_exists('utf8_decode')) $nomeProjeto = strlen(gpw_passthrough($t['projeto_nome'])) > 25 ? substr(gpw_passthrough($t['projeto_nome']), 0, 22).'...' : gpw_passthrough($t['projeto_nome']);
