...
function install_font_family($fontname, $normal, $bold = null, $italic = null, $bold_italic = null) {
  ...
  if ( mb_strtolower($ext) === ".pfb" || mb_strtolower($ext) === ".ttf" ) {
    ...
    foreach ($fonts as $var => $src) {
      ...
      if ( !is_readable($src) ) 
        throw new User_DOMPDF_Exception("Requested font '$caminhoname' is not readable");
   ...
   /* desbilitado por causa do fortfy 
  ...
  if ( mb_strtolower($ext) === ".ttf") {
    foreach ($fonts as $var => $font) {
      ...
      exec( _TTF2AFM . " " . escapeshellarg($font) . " " . $dest . " &> /dev/null", $saida, $ret );
...
$normal = previnirXSS($_SERVER["argv"][2]);
$bold   = isset($_SERVER["argv"][3]) ? previnirXSS($_SERVER["argv"][3]) : null;
$italic = isset($_SERVER["argv"][4]) ? previnirXSS($_SERVER["argv"][4]) : null;
$bold_italic = isset($_SERVER["argv"][5]) ? previnirXSS($_SERVER["argv"][5]) : null;

install_font_family(previnirXSS($_SERVER["argv"][1]), $normal, $bold, $italic, $bold_italic);
