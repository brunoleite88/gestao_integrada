...
class CPDF_Adapter implements Canvas {
  ...
  function text($x, $y, $text, $font, $size, $color = array(0,0,0),
    ...
    $this->_pdf->addText($x, $this->y($y) - Font_Metrics::get_font_height($font, $size), $size, gpw_passthrough($text), $angle, $adjust);
  ...
  function get_text_width($text, $font, $size, $spacing = 0) {
    $this->_pdf->selectFont($font);
    return $this->_pdf->getTextWidth($size, gpw_passthrough($text), $spacing);
  }
