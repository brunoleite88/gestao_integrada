...
function explode_url($url) {
  ...
  if ( isset($arr["scheme"]) &&
    ...
    if ( $path !== false ) {
      ...
      $host = isset($_SERVER["HTTP_HOST"]) ? previnirXSS($_SERVER["HTTP_HOST"]) : php_uname("n");

      if ( substr($arr["path"], 0, 1) == '/' ) {
        $path = dirname($arr["path"]);
      } else {
        $path = '/' . rtrim(dirname(previnirXSS($_SERVER["SCRIPT_NAME"])), '/') . '/' . $arr["path"];
      }
    }
  }
...
/**
 * Determines whether $value is a percentagem or not
 *
 * @param float $value
 * @return bool
 */
