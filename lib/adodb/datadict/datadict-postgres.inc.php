...
class ADODB2_postgres extends ADODB_DataDict {
	...
	function MetaType($t,$tamanho=-1,$fieldobj=false)
	{
		...
		$is_serial = is_object($fieldobj) && $fieldobj->primary_key && $fieldobj->unique && 
			$fieldobj->has_default && substr($fieldobj->valor_padrao,0,8) == 'nextval(';
	...
	function AddColumnSQL($tabname, $flds)
	{
		...
		foreach($lines as $v) {
			...
			if (preg_match('/^([^ ]+) .*DEFAULT ([^ ]+)/',$v,$comparados)) {
				list(,$colname,$default) = $comparados;
	   ...
	   if ($has_alter_column) {
	      ...
	      foreach($lines as $v) {
	         ...
	         if (preg_match('/^([^ ]+) .*DEFAULT ([^ ]+)/',$v,$comparados)) {
	            ...
	            // drop default?
	            preg_match ('/^\s*(\S+)\s+(.*)$/',$v,$comparados);
	            list (,$colname,$rest) = $comparados;
	...
	function _recreate_copy_table($tabname,$dropflds,$tableflds,$tableoptions='')
	{
		...
		foreach($this->MetaColumns($tabname) as $fld) {
			if (!$dropflds || !in_array($fld->name,$dropflds)) {
				...
				if (preg_match('/'.$fld->name.' (I|I2|I4|I8|N|F)/i',$tableflds,$comparados) && 
				...
				if ($fld->primary_key && $fld->has_default && 
					preg_match("/nextval\('([^']+)'::text\)/",$fld->valor_padrao,$comparados)) {
					$seq_name = $comparados[1];
					$seq_fld = $fld->name;
				}
			}
		}
