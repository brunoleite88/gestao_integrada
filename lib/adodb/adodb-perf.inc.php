...
V4.96 24 Sept 2007  (c) 2000-2007 John Lim (jlim#natsoft.com.my). All rights reserved.
  ...
  my needs. If you want to separate code from presentation, be my convidado. Patches
  are bemvindo.
...
function adodb_getmem()
{
	...
	if ( strncmp(strtoupper(PHP_OS),'WIN',3)==0) {
		...
		exec('tarefalist /FI "PID eq ' . $pid. '" /FO LIST', $saida); 
		return substr($saida[5], strpos($saida[5], ':') + 1);
	} 
	
	/* Hopefully UNIX */
	exec("ps --pid $pid --no-headers -o%mem,size", $saida);
	if (sizeof($saida) == 0) return 0;
	
	$memarr = explode(' ',$saida[0]);
	...
	global $ADODB_LOG_CONN;
		...
		if (!empty($ADODB_LOG_CONN)) {
			...
			if ($conn->databaseType != $connx->databaseType)
				$prefixo = '/*dbx='.$connx->databaseType .'*/ ';
			else
				$prefixo = '';
		} else {
			$conn =& $connx;
...
1: either
	...
	b. array holding sql string and field to look for, e.g. array('mostrar variables','table_cache'),
...
class adodb_perf {
	...
	var $titulos = '<tr><td><b>Parameter</b></td><td><b>Value</b></td><td><b>Description</b></td></tr>';
	var $warnRatio = 90;
	var $tablesSQL = false;
	var $cliFormat = "%32s => %s \r\n";
...
Committed_AS:   348732 kB
	...
	*/
	...
	function CPULoad()
	{
		...
		$ultimo = $this->_lastLoad;
		$this->_lastLoad = $info;
		
		$d_user = $info[0] - $ultimo[0];
		$d_nice = $info[1] - $ultimo[1];
		$d_system = $info[2] - $ultimo[2];
		$d_idle = $info[3] - $ultimo[3];
...
<table border=1 bgcolor=white><tr><td><b>Avg Time</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
			...
			while (!$rs->EOF) {
				...
				$prefixo = "<a target=sql".rand()." href=\"?hidem=1&exps=1&sql=".$raw."&x#explain\">";
				$sufixo = "</a>";
				if ($this->explain == false || strlen($prefixo)>$max) {
					$sufixo = ' ... <i>String too long for GET parameter: '.strlen($prefixo).'</i>';
					$prefixo = '';
				}
				$s .= "<tr><td>".adodb_round($rs->fields[0],6)."<td align=right>".$rs->fields[2]."<td><font size=-1>".$prefixo.htmlspecialchars($sql).$sufixo."</font>".
	...
	/*
		This relatorios the percentagem of load on the instance due to the most 
...
<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
			...
			while (!$rs->EOF) {
				...
				$prefixo = "<a target=sqle".rand()." href=\"?hidem=1&expe=1&sql=".$raw."&x#explain\">";
				$sufixo = "</a>";
				if($this->explain == false || strlen($prefixo>$max)) {
					$prefixo = '';
					$sufixo = '';
	...
	*/
	...
	if (isset($_GET['do'])) $do = $_GET['do'];
	 ...
	 else if (isset($_GET['sql'])) $do = 'versql';
	 else $do = 'stats';
	...
	if ($do == 'versql') $form = "<td><form># SQL:<input type=hidden value=versql name=do> <input type=text size=4 name=nsql value=$nsql><input type=submit value=Go></td></form>";
	else $form = "<td>&nbsp;</td>";
	...
	<a href=?do=stats><b>Performance Stats</b></a> &nbsp; <a href=?do=versql><b>View SQL</b></a>
	 &nbsp; <a href=?do=tables><b>View Tables</b></a> &nbsp; <a href=?do=poll><b>Poll Stats</b></a>",
	 $allowsql ? ' &nbsp; <a href=?do=fazerSQL><b>Run SQL</b></a>' : '',
	 "$form",
	 "</tr></table>";
		...
		case 'stats':
			...
			if (empty($ADODB_LOG_CONN))
				echo "<p>&nbsp; <a href=\"?do=versql&clearsql=1\">Clear SQL Log</a><br>";
		...
		case 'versql':
			if (empty($_GET['hidem']))
				echo "&nbsp; <a href=\"?do=versql&clearsql=1\">Clear SQL Log</a><br>";
	...
	*/
	function Poll($secs=5)
	{
		...
		//$this->conn->debug=1;
		if ($secs <= 1) $secs = 1;
		echo "Accumulating statistics, every $secs segundos...\n";flush();
...
<?php
   ...
   /************************************************************************/
    ...
    function OptimizeTables()
    {
        ...
        $modo = ADODB_OPT_LOW; 
        $ultimoArg = $args[ $numArgs - 1];
        if ( !is_string($ultimoArg)) {
            $modo = $ultimoArg;
            unset( $args[ $numArgs - 1]);
        }
        
        foreach( $args as $table) {
            $this->optimizeTable( $table, $modo);
        }
	}
