...
function ADODB_SetDatabaseAdapter(&$bd)
{
	global $_ADODB_ACTIVE_DBS;
		...
		foreach($_ADODB_ACTIVE_DBS as $k => $d) {
			if (PHP_VERSION >= 5) {
				...
				if ($d->db->_connectionID === $bd->_connectionID && $bd->database == $d->db->database) 
					return $k;
			}
		}
...
class ADODB_Active_Record {
	...
	// should be static
	function SetDatabaseAdapter(&$bd) 
	{
		return ADODB_SetDatabaseAdapter($bd);
	}
	
	// php4 constructor
	function ADODB_Active_Record($table = false, $pkeyarr=false, $bd=false)
	{
		ADODB_Active_Record::__construct($table,$pkeyarr,$bd);
	}
	
	// php5 constructor
	function __construct($table = false, $pkeyarr=false, $bd=false)
	{
	global $ADODB_ASSOC_CASE,$_ADODB_ACTIVE_DBS;
	
		if ($bd == false && is_object($pkeyarr)) {
			$bd = $pkeyarr;
			$pkeyarr = false;
		}
		...
		if ($bd) {
			$this->_dbat = ADODB_Active_Record::SetDatabaseAdapter($bd);
		} else
			$this->_dbat = sizeof($_ADODB_ACTIVE_DBS)-1;
		
		
		if ($this->_dbat < 0) $this->Error("No database connection set; use ADOdb_Active_Record::SetDatabaseAdapter(\$bd)",'ADODB_Active_Record::__constructor');
	...
	function _pluralize($table)
	{
		...
		$ultimoc = $ut[$tamanho-1];
		$ultimoc2 = substr($ut,$tamanho-2);
		switch ($ultimoc) {
		case 'S':
			return $table.'es';	
		case 'Y':
			return substr($table,0,$tamanho-1).'ies';
		case 'X':	
			return $table.'es';
		case 'H': 
			if ($ultimoc2 == 'CH' || $ultimoc2 == 'SH')
				return $table.'es';
		default:
			return $table.'s';
		}
	...
	global $ADODB_ASSOC_CASE,$_ADODB_ACTIVE_DBS , $ADODB_CACHE_DIR, $ADODB_ACTIVE_CACHESECS;
		...
		$bd =& $activedb->db;
		$fnome = $ADODB_CACHE_DIR . '/adodb_' . $bd->databaseType . '_active_'. $table . '.cache';
		if (!$forceUpdate && $ADODB_ACTIVE_CACHESECS && $ADODB_CACHE_DIR && file_exists($fnome)) {
			$fp = fopen($fnome,'r');
			@flock($fp, LOCK_SH);
			...
			if ($acttab->_created + $ADODB_ACTIVE_CACHESECS - (abs(rand()) % 16) > time()) { 
				...
				//if ($bd->debug) ADOConnection::outp("Reading cached active record file: $fnome");
			  	return;
			} else if ($bd->debug) {
				ADOConnection::outp("Refreshing cached active record file: $fnome");
			}
		}
		...
		if (!$pkeys) {
			if (isset($fld->primary_key)) {
				...
				$pkeys = $this->GetPrimaryKeys($bd, $table);
		}
		if (empty($pkeys)) {
		...
		if ($ADODB_ACTIVE_CACHESECS && $ADODB_CACHE_DIR) {
			...
			adodb_write_file($fnome,$s);
		}
		$activedb->tables[$table] = $activetab;
	}
	
	function GetPrimaryKeys(&$bd, $table)
	{
		return $bd->MetaPrimaryKeys($table);
	}
	...
	global $_ADODB_ACTIVE_DBS;
		...
		if (function_exists('adodb_throw')) {	
			if (!$bd) adodb_throw('ADOdb_Active_Record', $fn, -1, $err, 0, 0, false);
			else adodb_throw($bd->databaseType, $fn, -1, $err, 0, 0, $bd);
		} else
			if (!$bd || $bd->debug) ADOConnection::outp($this->_lasterr);
	...
	function ErrorMsg()
	{
		if (!function_exists('adodb_throw')) {
			...
			if ($bd && $bd->ErrorMsg()) return $bd->ErrorMsg();
		}
	...
	global $_ADODB_ACTIVE_DBS;
		...
		if ($this->_dbat < 0) {
			$false = false;
			$this->Error("No database connection set: use ADOdb_Active_Record::SetDatabaseAdaptor(\$bd)", "DB");
			return $false;
		}
		$activedb = $_ADODB_ACTIVE_DBS[$this->_dbat];
		$bd =& $activedb->db;
	...
	function LastInsertID(&$bd,$fieldname)
	{
		if ($bd->hasInsertID)
			$val = $bd->Insert_ID($this->_table,$fieldname);
		else
			$val = false;
			
		if (is_null($val) || $val === false) {
			// this might not work reliably in multi-user environment
			return $bd->GetOne("select max(".$fieldname.") from ".$this->_table);
		}
		return $val;
	}
	
	// quote data in where clause
	function doquote(&$bd, $val,$t)
	{
		switch($t) {
		case 'D':
		case 'T':
	...
	function GenWhere(&$bd, &$table)
	{
		...
		foreach($keys as $k) {
			$f = $table->flds[$k];
			if ($f) {
				$parr[] = $k.' = '.$this->doquote($bd,$this->$k,$bd->MetaType($f->type));
			}
		}
	...
	{
		$bd =& $this->DB(); if (!$bd) return false;
		$this->_where = $where;
		
		$save = $bd->SetFetchMode(ADODB_FETCH_NUM);
		$row = $bd->GetRow("select * from ".$this->_table.' WHERE '.$where,$bindarr);
		$bd->SetFetchMode($save);
	...
	{
		$bd =& $this->DB(); if (!$bd || empty($this->_table)) return false;
		$arr =& $bd->GetActiveRecordsClass(get_class($this),$this->_table, $whereOrderBy,$bindarr,$pkeysArr);
		return $arr;
	}
	...
	global $ADODB_ASSOC_CASE;
		...
		foreach($table->flds as $name=>$fld) {
			...
			if (is_null($val)) {
				if (isset($fld->not_null) && $fld->not_null) {
					if (isset($fld->valor_padrao) && strlen($fld->valor_padrao)) continue;
					else {
			...
			$t = $bd->MetaType($fld->type);
			$arr[$name] = $this->doquote($bd,$val,$t);
			$valarr[] = $val;
		}
		...
		$ok = $bd->Replace($this->_table,$arr,$pkey);
		if ($ok) {
