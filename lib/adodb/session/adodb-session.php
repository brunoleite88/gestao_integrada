...
/*
	You may want to rename the 'data' field to 'sessao_data' as
	...
	If you do, then execute:
		...
		ADODB_Session::dataFieldName('sessao_data');
...
function adodb_session_regenerate_id() 
{
	...
	if (function_exists('session_regenerate_id')) {
		...
		sessao_id(md5(uniqid(rand(), true)));
		$ck = session_get_cookie_params();
		setcookie(session_name(), sessao_id(), false, $ck['path'], $ck['domain'], $ck['secure']);
		//@session_start();
	}
	$new_id = sessao_id();
...
class ADODB_Session {
	...
	*/
	...
	/*!
	*/
	function syncSeconds($sync_segundos = null) {
		static $_sync_segundos = 60;
		static $set = false;

		if (!is_null($sync_segundos)) {
			$_sync_segundos = (int) $sync_segundos;
			$set = true;
		} elseif (!$set) {
			// backwards compatibility
	...
	*/
	...
	/*!
	*/
	function dataFieldName($data_campo_nome = null) {
		static $_data_campo_nome = 'data';

		if (!is_null($data_campo_nome)) {
			$_data_campo_nome = trim($data_campo_nome);
		}

		return $_data_campo_nome;
	}

	/*!
	*/
	function filter($filtro = null) {
	...
	*/
	function _sessionKey() {
		// use this function to create the encryption key for crypted sessoes
		// crypt the used key, ADODB_Session::encryptionKey() as key and sessao_id() as salt
		return crypt(ADODB_Session::encryptionKey(), sessao_id());
	}
...
*/
	...
	*/
	function gc($maxlifetime) 
	{
		...
		$sync_segundos	= ADODB_Session::syncSeconds();
		$table			= ADODB_Session::table();
		...
		if ($sync_segundos) {
			...
			if ($rs && !$rs->EOF) {
				...
				if (abs($dbt - $t) >= $sync_segundos) {
					$msg = __FILE__ .
						": Server time for webserver {$_SERVER['HTTP_HOST']} not in synch with database: " .
						" database=$dbt ($dbts), webserver=$t (diff=". (abs($dbt - $t) / 60) . ' minutos)';
					error_log($msg);
					if ($debug) {
