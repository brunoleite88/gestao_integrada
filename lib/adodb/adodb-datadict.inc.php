...
*/
function Lens_ParseArgs($args,$fimstmtchar=',',$tokenchars='_.-')
{
	$pos = 0;
	$intoken = false;
	$stmtno = 0;
...
class ADODB_DataDict {
	...
	{		
		...
		return $this->connection->MetaType($t,$tamanho,$fieldobj);
	}
	...
	function NameQuote($name = NULL,$allowBrackets=false)
	{
		...
		if ( preg_match('/^`(.+)`$/', $name, $comparados) ) {
			return $quote . $comparados[1] . $quote;
		}
	...
	*/
	...
	function CreateDatabase($nomeBd,$options=false)
	{
		$options = $this->_Options($options);
		$sql = array();
		
		$s = 'CREATE DATABASE ' . $this->NameQuote($nomeBd);
	...
	*/
	function CreateTableSQL($tabname, $flds, $tableoptions=array())
	{
        ...
        // we avoid passing REPLACE to trigger creation code. This preventos
	...
	function _GenFields($flds,$widespacing=false)
	{
		...
		foreach($flds as $fld) {
			...
			$fid = strtoupper(preg_replace('/^`(.+)`$/', '$1', $fnome));
			$fnome = $this->NameQuote($fnome);
			
			if (!strlen($ftype)) {
				if ($this->debug) ADOConnection::outp("Undefined TYPE for field '$fnome'");
				return false;
			} else {
			...
			$sufixo = $this->_CreateSuffix($fnome,$ftype,$fnotnull,$fdefault,$fautoinc,$fconstraint,$funsigned);
			
			// add index creation
			if ($widespacing) $fnome = str_pad($fnome,24);
			
			 // check for field names appearing twice
            if (array_key_exists($fid, $lines)) {
            	 ADOConnection::outp("Field '$fnome' defined twice");
            }
			
			$lines[$fid] = $fnome.' '.$ftype.$sufixo;
	...
	*/
	...
	function _CreateSuffix($fnome,$ftype,$fnotnull,$fdefault,$fautoinc,$fconstraint)
	{	
		$sufixo = '';
		if (strlen($fdefault)) $sufixo .= " DEFAULT $fdefault";
		if ($fnotnull) $sufixo .= ' NOT NULL';
		if ($fconstraint) $sufixo .= ' '.$fconstraint;
		return $sufixo;
	}
