    ...
    */
    function rs2xmlrpcval_header($adodbrs)
    {
        ...
        for ($i = 0; $i < $numfields; $i++) {
            ...
            if (isset($fld->valor_padrao))
                $fieldarray["valor_padrao"] =new xmlrpcval ($fld->valor_padrao);
    ...
    */
    function xmlrpcval2rs (&$xmlrpcval) {
        ...
        for ($i = 0; $i < $numfields; $i++) {
            ...
            while (list($key,$valor) = $temp->structeach()) {
                if ($key == "name") $fld->name = $valor->scalarval();
                if ($key == "type") $fld->type = $valor->scalarval();
                if ($key == "max_length") $fld->max_length = $valor->scalarval();
                if ($key == "not_null") $fld->not_null = $valor->scalarval();
                if ($key == "has_default") $fld->has_default = $valor->scalarval();
                if ($key == "valor_padrao") $fld->valor_padrao = $valor->scalarval();
            } // while
            $fields_array[] = $fld;
        } // for
