...
function adodb_probetypes(&$array,&$tipos,$probe=8)
{
// probe and guess the type
	...
	for ($j=0;$j < $max; $j++) {
		...
		foreach($row as $v) {
			...
			if (isset($tipos[$i]) && $tipos[$i]=='C') continue;
			
			//print " ($i ".$tipos[$i]. "$v) ";
			$v = trim($v);
			
			if (!preg_match('/^[+-]{0,1}[0-9\.]+$/',$v)) {
				$tipos[$i] = 'C'; // once C, always C
			...
			// that it is not an integer
				if (strlen($v) == 0) $tipos[$i] = 'C';
				if (strpos($v,'.') !== false) $tipos[$i] = 'N';
				else  $tipos[$i] = 'I';
				continue;
			}
...
function _array_change_key_case($an_array)
{
	if (is_array($an_array)) {
		$new_array = array();
		foreach($an_array as $key=>$valor)
			$new_array[strtoupper($key)] = $valor;
...
*/
		if ($hasvalue) 
			$valor = " value='".htmlspecialchars($zval2)."'";
		
		if (is_array($defstr))  {
			
			if (in_array($selected,$defstr)) 
				$s .= "\n<option selected='selected'$valor>".htmlspecialchars($zval).'</option>';
			else 
				$s .= "\n<option".$valor.'>'.htmlspecialchars($zval).'</option>';
		}
		else {
						...
						$inputarr=false, $secs2cache=0) 
{
	...
	$ultimopageno = (int) ceil($qryRecs / $nrows);
	...
	if ($rsreturn) {
		...
		$rsreturn->LastPageNo($ultimopageno);
	}
	return $rsreturn;
}
			...
			if (adodb_key_exists($upperfname,$arrFields,$force)) {
				...
				if ($forceUpdate || strcmp($val, $arrFields[$upperfname])) {
					...
					if ((strpos($upperfname,' ') !== false) || ($ADODB_QUOTE_FIELDNAMES))
						$fnomeq = $zthis->nameQuote.$upperfname.$zthis->nameQuote;
					else
						$fnomeq = $upperfname;
                    ...
                    )
                {
                    switch ($force) {
                        ...
                        case 2:
                            ...
                            $setFields .= _adodb_column_sql($zthis, 'U', $type, $upperfname, $fnomeq,$arrFields, $magicq);
                        break;
						default:
                        case 3:
                ...
                //********************************************************//
						...
						$setFields .= _adodb_column_sql($zthis, 'U', $type, $upperfname, $fnomeq,
														  $arrFields, $magicq);
					}
				}
			}
...
static $cacheCols;
	...
	foreach( $columns as $field ) { 
		...
		if (adodb_key_exists($upperfname,$arrFields,$force)) {
				...
				)
               {
                    switch ($force) {
                        ...
                        case 2:
                            //Set empty
                            $arrFields[$upperfname] = "";
                            $valors .= _adodb_column_sql($zthis, 'I', $type, $upperfname, $fnomeq,$arrFields, $magicq);
                        break;
            ...
            /*********************************************************/
				...
				//Oracle needs BLOB types to be handled with a returning clause
				//postgres has special needs as well
				$valors .= _adodb_column_sql($zthis, 'I', $type, $upperfname, $fnomeq,
											   $arrFields, $magicq);
			}
	...
	$valors = substr($valors, 0, -2);

	// Append the fields and their values to the insert query.
	return 'INSERT INTO '.$tableName.' ( '.$fields.' ) VALUES ( '.$valors.' )';
}
 ...
 * 
 */
function _adodb_column_sql_oci8(&$zthis,$action, $type, $fnome, $fnomeq, $arrFields, $magicq) 
{
    $sql = '';
    ...
    case 'B':
        ...
        if (!empty($zthis->hasReturningInto)) {
            ...
            $sql = _adodb_column_sql($zthis, $action, $type, $fnome, $fnomeq, $arrFields, $magicq,false);
        }
        break;
...
function _adodb_column_sql(&$zthis, $action, $type, $fnome, $fnomeq, $arrFields, $magicq, $recurse=true) 
{
	...
	if ($recurse) {
		...
		case 'postgres':
			...
			break;
		case 'oci8':
			return _adodb_column_sql_oci8($zthis, $action, $type, $fnome, $fnomeq, $arrFields, $magicq);
	...
	switch($type) {
		...
		case "X":
		case 'B':
			$val = $zthis->qstr($arrFields[$fnome],$magicq);
			break;

		case "D":
			$val = $zthis->DBDate($arrFields[$fnome]);
			break;

		
		case "T":
			$val = $zthis->DBTimeStamp($arrFields[$fnome]);
            break;
		...
		default:
			$val = str_replace(array("'"," ","("),"",$arrFields[$fnome]); // basic sql injection defence
			if (empty($val)) $val = '0';
			break;
	}
...
function _adodb_backtrace($printOrArr=true,$nivels=9999,$skippy=0)
{
...
function _adodb_find_from($sql) 
{
	...
	for($i = 0; $i < $charCount; $i++) {
		...
		elseif($char == ")" && $inString === false && $superiorheseCount > 0)
			$superiorheseCount--;

		elseif($superiorheseCount <= 0 && $inString === false && $char == " " && strtoupper(substr($prevChars,-5,5)) == " FROM")
			return $i;
