...
* Options are INSERT (attempts to insert duplicar rows), UPDATE (updates existing
...
class dbObject {
	...
	* NOP
	*/
	function dbObject( &$superior, $attributes = NULL ) {
...
class dbTable extends dbObject {
	...
	* @access private
	*/
	var $atualPlatform = true;
	
	
	/**
	* Iniitializes a new table object.
	*
	* @param string $prefixo DB Object prefix
	* @param array $attributes Array of table attributes.
	*/
	function dbTable( &$superior, $attributes = NULL ) {
		$this->parent =& $superior;
	...
	*/
	function addField( $name, $type, $size = NULL, $opts = NULL ) {
		$campo_id = $this->FieldID( $name );
		
		// Set the field index so we know where we are
		$this->current_field = $campo_id;
		
		// Set the field name (required)
		$this->fields[$campo_id]['NAME'] = $name;
		
		// Set the field type (required)
		$this->fields[$campo_id]['TYPE'] = $type;
		
		// Set the field size (optional)
		if( isset( $size ) ) {
			$this->fields[$campo_id]['SIZE'] = $size;
		}
		
		// Set the field options
		if( isset( $opts ) ) {
			$this->fields[$campo_id]['OPTS'] = array($opts);
		} else {
			$this->fields[$campo_id]['OPTS'] = array();
		}
	}
	...
	* @param mixed $valor Field option value
	* @return array Field specifier array
	*/
	function addFieldOpt( $field, $opt, $valor = NULL ) {
		if( $this->currentPlatform ) {
		if( !isset( $valor ) ) {
			$this->fields[$this->FieldID( $field )]['OPTS'][] = $opt;
		// Add the option and value
		} else {
			$this->fields[$this->FieldID( $field )]['OPTS'][] = array( $opt => $valor );
		}
	}
	}
	...
	*/
	function create( &$xmls ) {
		...
		if( is_array( $legacy_fields = $xmls->dict->MetaColumns( $this->name ) ) ) {
			...
			foreach( $legacy_fields as $campo_id => $field ) {
				if( !isset( $this->fields[$campo_id] ) ) {
		...
		foreach( $this->fields as $campo_id => $finfo ) {
			...
			if( isset( $finfo['OPTS'] ) ) {
				foreach( $finfo['OPTS'] as $opt ) {
					...
					if( is_array( $opt ) ) {
						$key = key( $opt );
						$valor = $opt[key( $opt )];
						@$fldarray[$campo_id][$key] .= $valor;
					// Option doesn't have arguments
					} else {
						$fldarray[$campo_id][$opt] = $opt;
					}
				}
			}
		}
...
class dbIndex extends dbObject {
	...
	*
	* @param object $superior Parent object
	* @param array $attributes Attributes
	*
	* @internal
	*/
	function dbIndex( &$superior, $attributes = NULL ) {
		$this->parent =& $superior;
...
class dbData extends dbObject {
	...
	* @internal
	*/
	function dbData( &$superior, $attributes = NULL ) {
		$this->parent =& $superior;
	}
	...
	*/
	function create( &$xmls ) {
		...
		foreach( $this->data as $row ) {
			...
			foreach( $row as $campo_id => $field_data ) {
				if( !array_key_exists( $campo_id, $table_fields ) ) {
					if( is_numeric( $campo_id ) ) {
						$campo_id = reset( array_keys( $table_fields ) );
					} else {
						continue;
					}
				}
				
				$name = $table_fields[$campo_id]['NAME'];
				
				switch( $table_fields[$campo_id]['TYPE'] ) {
					case 'I':
					case 'I1':
					case 'I2':
					case 'I4':
			...
			if( !in_array( $table, $tables ) or ( $modo = $xmls->existingData() ) == XMLS_MODE_INSERT ) {
...
class dbQuerySet extends dbObject {
	...
	* @param array $attributes Attributes
	*/
	function dbQuerySet( &$superior, $attributes = NULL ) {
		$this->parent =& $superior;
	...
	*/
	function create( &$xmls ) {
		foreach( $this->queries as $id => $comando_sql ) {
			switch( $this->prefixMethod ) {
				case 'AUTO':
					...
					$comando_sql = $this->prefixQuery( '/^\s*((?is)INSERT\s+(INTO\s+)?)((\w+\s*,?\s*)+)(\s.*$)/', $comando_sql, $xmls->objectPrefix );
					$comando_sql = $this->prefixQuery( '/^\s*((?is)UPDATE\s+(FROM\s+)?)((\w+\s*,?\s*)+)(\s.*$)/', $comando_sql, $xmls->objectPrefix );
					$comando_sql = $this->prefixQuery( '/^\s*((?is)DELETE\s+(FROM\s+)?)((\w+\s*,?\s*)+)(\s.*$)/', $comando_sql, $xmls->objectPrefix );
					
					// SELECT statements aren't working yet
					#$data = preg_replace( '/(?ias)(^\s*SELECT\s+.*\s+FROM)\s+(\W\s*,?\s*)+((?i)\s+WHERE.*$)/', "\1 $prefixo\2 \3", $data );
				...
				case 'MANUAL':
					...
					if( isset( $this->prefixKey ) AND( $this->prefixKey !== '' ) ) {
						...
						$comando_sql = str_replace( $this->prefixKey, $xmls->objectPrefix, $comando_sql );
					} else {
						// Use default replacement
						$comando_sql = str_replace( XMLS_PREFIX , $xmls->objectPrefix, $comando_sql );
					}
			}
			
			$this->queries[$id] = trim( $comando_sql );
		}
	...
	* @param string $comando_sql SQL query string
	* @param string $prefixo Prefix to be appended to tables, indices, etc.
	* @return string Prefixed SQL query string.
	*/
	function prefixQuery( $regex, $comando_sql, $prefixo = NULL ) {
		if( !isset( $prefixo ) ) {
			return $comando_sql;
		}
		
		if( preg_match( $regex, $comando_sql, $comparar ) ) {
			$preamble = $comparar[1];
			$postamble = $comparar[5];
			$objectList = explode( ',', $comparar[3] );
			// $prefixo = $prefixo . '_';
			...
			foreach( $objectList as $object ) {
				...
				$prefixedList .= $prefixo . trim( $object );
			}
			
			$comando_sql = $preamble . ' ' . $prefixedList . ' ' . $postamble;
		}
...
class adoSchema {
	...
	* have been created.
	*
	* @param object $bd ADOdb database connection object.
	*/
	function adoSchema( &$bd ) {
	...
	* to always insert the data as a new row. In the evento of a unique key violation,
	...
	*
	* @param int $modo XMLS_MODE_INSERT, XMLS_MODE_UPDATE, or XMLS_MODE_IGNORE
	* @return int current mode
	*/
	function ExistingData( $modo = NULL ) {
		if( is_int( $modo ) ) {
			switch( $modo ) {
				case XMLS_MODE_UPDATE:
					$modo = XMLS_MODE_UPDATE;
					break;
				case XMLS_MODE_IGNORE:
					$modo = XMLS_MODE_IGNORE;
					break;
				case XMLS_MODE_INSERT:
					$modo = XMLS_MODE_INSERT;
					break;
				default:
					$modo = XMLS_EXISITNG_DATA;
					break;
			}
			$this->existingData = $modo;
		}
	...
	*
	* @param bool $modo execute
	* @return bool current execution mode
	*
	* @see ParseSchema(), ExecuteSchema()
	*/
	function ExecuteInline( $modo = NULL ) {
		if( is_bool( $modo ) ) {
			$this->executeInline = $modo;
		}
	...
	*/
	function ContinueOnError( $modo = NULL ) {
		if( is_bool( $modo ) ) {
			$this->continueOnError = $modo;
		}
	...
	* with large schema arquivos. Otherwise, ParseSchema() is faster.
	...
	*/
	function SaveSQL( $filename = './schema.sql' ) {
		...
		foreach( $sqlArray as $key => $comando_sql ) {
			fwrite( $fp, $comando_sql . ";\n" );
		}
		fclose( $fp );
	}
	...
	*/
	function ConvertSchemaString( $schema, $newVersion = NULL, $newFile = NULL ) {
		...
		if( $version == $newVersion ) {
			...
			$resultado = $this->TransformSchema( $schema, 'convert-' . $version . '-' . $newVersion);
		}
		
		if( is_string( $resultado ) AND is_string( $newFile ) AND ( $fp = fopen( $newFile, 'w' ) ) ) {
			fwrite( $fp, $resultado );
			fclose( $fp );
		}
	...
	function _file_get_contents($caminho)
	{
		if (function_exists('file_get_contents')) return file_get_contents($caminho);
		return join('',file($caminho));
	}*/
	...
	*/
	function ConvertSchemaFile( $filename, $newVersion = NULL, $newFile = NULL ) {
		...
		if( $version == $newVersion ) {
			$resultado = _file_get_contents( $filename );
			
			// remove unicode BOM if present
			if( substr( $resultado, 0, 3 ) == sprintf( '%c%c%c', 239, 187, 191 ) ) {
				$resultado = substr( $resultado, 3 );
			}
		} else {
			$resultado = $this->TransformSchema( $filename, 'convert-' . $version . '-' . $newVersion, 'file' );
		}
	...
	function TransformSchema( $schema, $xsl, $schematype='string' )
	{
		...
		// process the schema
		$resultado = xslt_process ($xh, 'arg:/_xml', 'arg:/_xsl', NULL, $arguments); 
	...
	* @param integer $nivel Error level
	* @param array $fields Error information fields
	*
	* @access private
	*/
	function xslt_error_handler( $parser, $errno, $nivel, $fields ) {
		if( is_array( $fields ) ) {
			$msg = array(
		...
		$erro_details = $msg['Message Type'] . ' in XSLT Transformation' . "\n"
					   . '<table>' . "\n";
		
		foreach( $msg as $legenda => $details ) {
			$erro_details .= '<tr><td><b>' . $legenda . ': </b></td><td>' . htmlentities( $details ) . '</td></tr>' . "\n";
		}
		
		$erro_details .= '</table>';
		
		trigger_error( $erro_details, E_USER_ERROR );
	}
	...
	*/
	function SchemaFileVersion( $filename ) {
		...
		// Process the file
		while( $data = fread( $fp, 4096 ) ) {
			if( preg_match( $this->versionRegex, $data, $comparados ) ) {
				return !empty( $comparados[2] ) ? $comparados[2] : XMLS_DEFAULT_SCHEMA_VERSION;
			}
		}
	...
	*/
	function SchemaStringVersion( $xmlstring ) {
		...
		if( preg_match( $this->versionRegex, $xmlstring, $comparados ) ) {
			return !empty( $comparados[2] ) ? $comparados[2] : XMLS_DEFAULT_SCHEMA_VERSION;
		}
	...
	*/
	function ExtractSchema( $data = FALSE, $indent = '  ', $prefixo = '' , $stripprefix=false) {
		$old_mode = $this->db->SetFetchMode( ADODB_FETCH_NUM );
		
		$schema = '<?xml version="1.0"?>' . "\n"
				. '<schema version="' . $this->schemaVersion . '">' . "\n";
		
		if( is_array( $tables = $this->db->MetaTables( 'TABLES' , ($prefixo) ? $prefixo.'%' : '') ) ) {
				...
				if( is_array( $fields ) ) {
					foreach( $fields as $details ) {
						...
						if( isset($details->has_default) && $details->has_default ) {
							$content[] = '<DEFAULT value="' . htmlentities( $details->valor_padrao ) . '"/>';
						}
	...
	function SetPrefix( $prefixo = '', $underscore = TRUE ) {
		switch( TRUE ) {
			...
			case empty( $prefixo ):
				...
				return TRUE;
			// prefix too long
			case strlen( $prefixo ) > XMLS_PREFIX_MAXLEN:
			// prefix contains invalid characters
			case !preg_match( '/^[a-z][a-z0-9_]+$/i', $prefixo ):
				logMsg( 'Invalid prefix: ' . $prefixo );
				return FALSE;
		}
		
		if( $underscore AND substr( $prefixo, -1 ) != '_' ) {
			$prefixo .= '_';
		}
		
		// prefix valid
		logMsg( 'Set prefix: ' . $prefixo );
		$this->objectPrefix = $prefixo;
		return TRUE;
	}
...
* @access private
*/
function logMsg( $msg, $titulo = NULL, $force = FALSE ) {
	if( XMLS_DEBUG or $force ) {
		echo '<pre>';
		
		if( isset( $titulo ) ) {
			echo '<h3>' . htmlentities( $titulo ) . '</h3>';
		}
