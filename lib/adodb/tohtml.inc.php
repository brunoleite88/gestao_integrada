...
//	$bd->Connect('mysql','userid','password','database');
//	$rs = $bd->Execute('select col1,col2,col3 from table');
...
GLOBAL $gSQLMaxRows,$gSQLBlockRows,$ADODB_ROUND;
	...
	for ($i=0; $i < $ncols; $i++) {	
		...
		if ($field) {
			if ($zheaderarray) $fnome = $zheaderarray[$i];
			else $fnome = htmlspecialchars($field->name);	
	...
	// additional EOF check to prevento a widow header
