 ...
 * @mostrarcount		Mostrar count of records
 *
 * @returns			Sql generated
 */
 
 function PivotTableSQL(&$bd,$tables,$rowfields,$colfield, $where=false,
 	$aggfield = false,$sumlabel='Sum ',$aggfn ='SUM', $mostrarcount = true)
 {
	if ($aggfield) $hidecnt = true;
	else $hidecnt = false;
	
	$iif = strpos($bd->databaseType,'access') !== false; 
		// note - vfp 6 still doesn' work even with IIF enabled || $bd->databaseType == 'vfp';
	
	//$hidecnt = false;
	
 	if ($where) $where = "\nWHERE $where";
	if (!is_array($colfield)) $colarr = $bd->GetCol("select distinct $colfield from $tables $where order by 1");
	...
	if (is_array($colfield)) {
		...
		foreach ($colarr as $v) {
			...
			if ($aggfield) {
				if ($hidecnt) $legenda = $v;
				else $legenda = "{$v}_$aggfield";
				$sel .= $iif ?
					"\n\t$aggfn(IIF($colfield=$vq,$aggfield,0)) AS \"$legenda\", "
					:
					"\n\t$aggfn(CASE WHEN $colfield=$vq THEN $aggfield ELSE 0 END) AS \"$legenda\", ";
			}
		}
	}
...
# Set the rows to CiaName and QuantityPerUnit
...
#
 ...
 $sql = PivotTableSQL(
	...
	'CiaName,QuantityPerUnit',					# row fields
...
SELECT CiaName,QuantityPerUnit, 
...
GROUP BY CiaName,QuantityPerUnit
*/
//=====================================================================
