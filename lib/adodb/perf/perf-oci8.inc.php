...
AND    b.name = 'sorts (memory)'",
	...
	'Memory Pools',
		...
		'large pool buffer size' => array('CACHE',
			...
			'this pool is for large mem allocations (not because it is larger than shared pool), for MTS sessoes, parallel queries, io buffers (large_pool_size) ' ),
		...
		'max connections' => array( 'SESS',
			"select value from v\$parameter where name='sessoes'",
			''),
	...
	'Transactions',
		...
		'peak transactions' => array('ROLLBACK',
    		...
    		from sys.v_\$recurso_limit
    		where recurso_nome = 'transactions'",
...
FROM v\$parameter v1, v\$parameter v2 WHERE v1.name='log_archive_dest' AND v2.name='log_archive_dest_10'", ''),
	...
	'Flashback Area' => array('BACKUP', "select nvl(value,'Flashback Area not used') from v\$parameter where name=lower('DB_RECOVERY_FILE_DEST')", 'Flashback area is a folder where all backup data and logs can be stored and managed by Oracle. If Error: message displayed, then it is not in use.'),
		'Control File Keep Time' => array('BACKUP', "select value from v\$parameter where name='control_file_record_keep_time'",'No of dias to keep RMAN info in control file. I recommend it be set to x2 or x3 times the frequency of your full backup.'),
		false
	...
	function PGA()
	{
	   ...
	   (select round(pga_target_for_estimate/1024.0/1024.0,0) Mb,
	   	   pga_target_factor targ,estd_pga_cache_hit_percentagem pct,rownum as r 
...
select  a.size_for_estimate as cache_mb_estimate,
   ...
   (select size_for_estimate,size_factor,estd_physical_read_factor,rownum r from v\$db_cache_advice) b where a.r = b.r-1");
		...
		/*
		The v$db_cache_advice utility mostrar the marginal changes in physical data block reads for different sizes of db_cache_size
		*/
	...
	function tohtml(&$rs,$type)
	{
		...
		while (!$rs->EOF) {
			if ($check != $rs->fields[0].'::'.$rs->fields[1]) {
				if ($check) {
					$carr = explode('::',$check);
					$prefixo = "<a href=\"?$type=1&sql=".rawurlencode($sql).'&x#explain">';
					$sufixo = '</a>';
					if (strlen($prefixo)>2000) {
						$prefixo = '';
						$sufixo = '';
					}
					
					$s .=  "\n<tr><td align=right>".$carr[0].'</td><td align=right>'.$carr[1].'</td><td>'.$prefixo.$sql.$sufixo.'</td></tr>';
				}
				$sql = $rs->fields[2];
		...
		$carr = explode('::',$check);
		$prefixo = "<a target=".rand()." href=\"?&hidem=1&$type=1&sql=".rawurlencode($sql).'&x#explain">';
		$sufixo = '</a>';
		if (strlen($prefixo)>2000) {
			$prefixo = '';
			$sufixo = '';
...
from
  ( 
	...
	select
	  ...
	  buffer_gets,
	  executions,
	  100 * ratio_to_relatorio(buffer_gets) over ()  pct
	from
	  sys.v_\$sql
	where
	  command_type != 47 and modulo != 'T.O.A.D.'
      )
    where
...
from
  ( 
	...
	select
	  ...
	  disk_reads,
	  executions,
	  100 * ratio_to_relatorio(disk_reads) over ()  pct
	from
	  sys.v_\$sql
	where
	  command_type != 47 and modulo != 'T.O.A.D.'
