...
Set tabs to 4 for best vering.
  ...
  Requires ODBC. Works on Windows and Unix.
	...
	Problems: 
		Where is float/decimal type in pfazer_param_type
...
class ADODB_pfazer_base extends ADODB_pdo {

	var $sysDate = "'?'";
	var $sysTimeStamp = "'?'";
	

	function _init($superiorDriver)
	{
		$superiorDriver->_bindInputArray = true;
		#$superiorDriver->_connectionID->setAttribute(PDO::MYSQL_ATTR_USE_BUFFERED_QUERY,true);
	}
...
class ADODB_pdo extends ADOConnection {
	...
	function _connect($argDSN, $argUsername, $argPassword, $argDatabasename, $persist=false)
	{
		...
		if ($this->_connectionID) {
			...
			$classe = 'ADODB_pfazer_'.$this->dsnType;
			...
			case 'pgsql':
			case 'mssql':
				include_once(ADODB_DIR.'/drivers/adodb-pfazer_'.$this->dsnType.'.inc.php');
				break;
			}
			if (class_exists($classe))
				$this->_driver = new $classe();
			else
				$this->_driver = new ADODB_pfazer_base();
	...
	{
		return $this->_driver->MetaTables($ttype,$mostrarSchema,$mask);
	}
...
class ADORecordSet_pdo extends ADORecordSet {	
	...
	function ADORecordSet_pdo($id,$modo=false)
	{
		if ($modo === false) {  
		...
		switch($modo) {
		case ADODB_FETCH_NUM: $modo = PDO::FETCH_NUM; break;
		case ADODB_FETCH_ASSOC:  $modo = PDO::FETCH_ASSOC; break;
		
		case ADODB_FETCH_BOTH: 
		default: $modo = PDO::FETCH_BOTH; break;
		}
		$this->fetchMode = $modo;
	...
	#		$false = false;
		...
		else $o->type = adodb_pfazer_type($arr['pfazer_type']);
