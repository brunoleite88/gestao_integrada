...
thanks a lot for your help. Todia we discovered what
...
the whole query speed up from 1 till 10 segundos
to 0.2 till 0.3 segundos for 100 records. Amazing!!!
...
$bd = NewADOConnection('odbc_db2');
$bd->curMode = SQL_CUR_USE_ODBC;
$bd->Connect($dsn, $userid, $pwd);
...
I have had relatorios that the $host and $database params have to be reversed in 
...
class ADODB_ODBC_DB2 extends ADODB_odbc {
	...
	function &MetaTables($ttype=false,$mostrarSchema=false, $qtable="%", $qschema="%")
	{
	global $ADODB_FETCH_MODE;
		...
		for ($i=0; $i < sizeof($arr); $i++) {
			...
			if ($mostrarSchema) $arr[$i][2] = $arr[$i][1].'.'.$arr[$i][2];
	...
	// use right() and replace() ?
		...
		for ($i=0; $i < $tamanho; $i++) {
			...
			case 'y':
				...
				break;
			case 'M':
				$s .= "substr(mesname($col),1,3)";
				break;
			case 'm':
				$s .= "right(digits(mes($col)),2)";
				break;
			case 'D':
			case 'd':
				$s .= "right(digits(dia($col)),2)";
				break;
			case 'H':
			case 'h':
				if ($col != $this->sysDate) $s .= "right(digits(hora($col)),2)";	
				else $s .= "''";
				break;
			case 'i':
			case 'I':
				if ($col != $this->sysDate)
					$s .= "right(digits(minuto($col)),2)";
					else $s .= "''";
				break;
			case 'S':
			case 's':
				if ($col != $this->sysDate)
					$s .= "right(digits(segundo($col)),2)";
				else $s .= "''";
				break;
			default:
				if ($ch == '\\') {
...
class  ADORecordSet_odbc_db2 extends ADORecordSet_odbc {	
	...
	function ADORecordSet_db2($id,$modo=false)
	{
		$this->ADORecordSet_odbc($id,$modo);
	}
