...
class ADODB_mysqli extends ADOConnection {
  ...
  var $arrayClass = 'ADORecordSet_array_mysqli';
	...
	// pass get_magic_quotes_gpc() as the segundo parameter. This will 
	// ensure that the variable is not quoted twice, once by qstr and once 
	// by the magic_quotes_gpc.
	//
	//Eg. $s = $bd->qstr(_GET['name'],get_magic_quotes_gpc());
	...
	function _insertid()
	{
	  $resultado = @mysqli_insert_id($this->_connectionID);
	  if ($resultado == -1){
	...
	{
	  $resultado =  @mysqli_affected_rows($this->_connectionID);
	  if ($resultado == -1) {
  	...
  	function &MetaDatabases()
	{
		...
		$ret =& $this->Execute($comando_sql);
		if ($ret && is_object($ret)){
		   $arr = array();
			while (!$ret->EOF){
				$bd = $ret->Fields('Database');
				if ($bd != 'mysql') $arr[] = $bd;
				$ret->MoveNext();
			}
   		   return $arr;
	...
	// diaFraction is a dia in floating point
...
//	    $this->_connectionID = $this->mysqli_resolve_link($this->_connectionID);
	    ...
	    if ($this->_connectionID) {
        	$resultado = @mysqli_select_db($this->_connectionID, $dbName);
			if (!$resultado) {
	...
	*/
	...
	// for instance, you may have UTF8 database and server it on-site as utf8mb4 etc.
...
class ADORecordSet_mysqli extends ADORecordSet{	
	...
	function ADORecordSet_mysqli($queryID, $modo = false) 
	{
	  if ($modo === false) 
	   { 
	      global $ADODB_FETCH_MODE;
...
class ADORecordSet_array_mysqli extends ADORecordSet_array {
  function ADORecordSet_array_mysqli($id=-1,$modo=false) 
  {
