...
class ADODB_sybase_ase extends ADODB_sybase {
	 ...
	 var $metaColumnsSQL = "SELECT syscolumns.name AS campo_nome, systypes.name AS type, systypes.length AS width FROM sysobjects, syscolumns, systypes WHERE sysobjects.name='%s' AND syscolumns.id = sysobjects.id AND systypes.type=syscolumns.type";
	...
	// fix a bug which prevento the metaColumns query to be executed for Sybase ASE
	...
	function ErrorMsg()
	{
		if (!function_exists('sybase_connect')){
				return 'Your PHP doesn\'t contain the Sybase connection modulo!';
		}
...
var $databaseType = "sybase_ase";
function ADORecordset_sybase_ase($id,$modo=false)
	{
		$this->ADORecordSet_sybase($id,$modo);
	}
