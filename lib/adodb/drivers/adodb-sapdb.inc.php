...
class ADODB_SAPDB extends ADODB_odbc {
	...
	function ServerInfo()
	{
		...
		if (!$info['version'] && preg_match('/([0-9.]+)/',$info['description'],$comparados)) {
			$info['version'] = $comparados[1];
		}
		return $info;
	}
				...
				if ($fld->primary_key && $column[6] == 'DEFAULT SERIAL (1)') {
					...
					switch($fld->type) {
						...
						case 'LONG':
							...
							break;
						default:
							$fld->valor_padrao = trim($column[6]);
							break;
					}
				}
			}
...
class  ADORecordSet_sapdb extends ADORecordSet_odbc {	
	...
	function ADORecordSet_sapdb($id,$modo=false)
	{
