...
function adodb_addslashes($s)
{
	$tamanho = strlen($s);
	if ($tamanho == 0) return "''";
	if (strncmp($s,"'",1) === 0 && substr($s,$tamanho-1) == "'") return $s; // already quoted
...
class ADODB_postgres64 extends ADOConnection{
	...
	union 
        select vername,'V' from pg_vers where vername not like 'pg\_%'";
...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
	...
	{
		$resultado=pg_exec($this->_connectionID, "SELECT last_value FROM ${tablename}_${fieldname}_seq");
		if ($resultado) {
...
// Warning: This compilation does not support pg_cmdtuples() in adodb-postgres.inc.php on line 44
   function _affectedrows()
   {
	...
	union 
        select vername,'V' from pg_vers where vername not like 'pg\_%'  and schemaname  not in ( 'pg_catalog','information_schema') ";
		}
		if ($mask) {
...
select tablename,'T' from pg_tables where tablename like $mask and schemaname not in ( 'pg_catalog','information_schema')  
 union 
select vername,'V' from pg_vers where vername like $mask and schemaname  not in ( 'pg_catalog','information_schema')  ";
			else
				$this->metaTablesSQL = "
select tablename,'T' from pg_tables where tablename like $mask 
 union 
select vername,'V' from pg_vers where vername like $mask";
		}
	...
	*/ 
	...
	{ 
		...
		$fd = fopen($caminho,'r');
		$contents = fread($fd,filesize($caminho));
		fclose($fd);
	...
	* added maxsize parameter, which defaults to $bd->maxblobsize if not defined.
	*/ 
	...
	*/
	...
	{		
		if (!$data) $data = $this->sysDate;
			...
			$tamanho = strlen($data);
			if (10 <= $tamanho && $tamanho <= 12) $data = 'date '.$data;
			else $data = 'timestamp '.$data;
		}
		return "($data+interval'$diaFraction dias')";
	}
	...
	global $ADODB_FETCH_MODE;
		...
		while (!$rs->EOF) { 	
			...
			if ($fld->has_default) {
				$fld->valor_padrao = $rsdefa[$rs->fields[6]];
			}
...
WHERE (c2.relname=\'%s\' or c2.relname=lower(\'%s\'))';
	...
	//
	// examples:
	// 	$bd->Connect("host=host1 user=user1 password=secret port=4341");
	// 	$bd->Connect('host1','user1','secret');
	function _connect($str,$user='',$pwd='',$bd='',$ctype=0)
	{
		...
		if ($user || $pwd || $bd) {
			...
			if (strlen($bd) == 0) $bd = 'template1';
			$bd = adodb_addslashes($bd);
		   	if ($str)  {
			 	$host = split(":", $str);
		...
		//if ($user) $linea = "user=$user host=$linea password=$pwd nomeBd=$bd port=5432";
	...
	//
	// examples:
	// 	$bd->PConnect("host=host1 user=user1 password=secret port=4341");
	// 	$bd->PConnect('host1','user1','secret');
	function _pconnect($str,$user='',$pwd='',$bd='')
	{
		return $this->_connect($str,$user,$pwd,$bd,1);
	}
...
class ADORecordSet_postgres64 extends ADORecordSet{
	...
	var $databaseType = "postgres64";
	var $canSeek = true;
	function ADORecordSet_postgres64($queryID,$modo=false) 
	{
		if ($modo === false) { 
			global $ADODB_FETCH_MODE;
