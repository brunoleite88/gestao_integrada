...
class ADODB_ibase extends ADOConnection {
	...
	var $ibase_datefmt = '%Y-%m-%d'; // For horas,mins,secs change to '%Y-%m-%d %H:%M:%S';
	var $fmtDate = "'Y-m-d'";
	...
	//OPN STUFF start
	var $metaColumnsSQL = "select a.rdb\$campo_nome, a.rdb\$null_flag, a.rdb\$default_source, b.rdb\$field_length, b.rdb\$field_scale, b.rdb\$field_sub_type, b.rdb\$field_precision, b.rdb\$field_type from rdb\$relation_fields a, rdb\$fields b where a.rdb\$field_source = b.rdb\$campo_nome and a.rdb\$relation_name = '%s' order by a.rdb\$field_position asc";
	//OPN STUFF end
	var $ibasetrans;
	var $hasGenID = true;
	...
	{  
		...
		if ($this->perfil)
			$this->_connectionID = $fn($argHostname,$argUsername,$argPassword,
					$this->charSet,$this->buffers,$this->dialect,$this->perfil);
		else	
			$this->_connectionID = $fn($argHostname,$argUsername,$argPassword,
	...
	function Prepare($sql)
	{
	   ...
	   // there have been relatorios of problems with nested queries - the code is probably not re-entrant?
	...
	global $ADODB_FETCH_MODE;
		...
		while (!$rs->EOF) { //print_r($rs->fields);
			...
			if (isset($rs->fields[2])) {
				...
				{
				case 'smallint':
				case 'integer': $fld->valor_padrao = (int) $d; break;
				case 'char': 
				case 'blob':
				case 'text':
				case 'varchar': $fld->valor_padrao = (string) substr($d,1,strlen($d)-2); break;
				case 'double':
				case 'float': $fld->valor_padrao = (float) $d; break;
				default: $fld->valor_padrao = $d; break;
				}
	...
	*/
	...
	$tamanho = strlen($val); 
	$chunk_size = 32768; 
	$tail_size = $tamanho % $chunk_size; 
	$n_chunks = ($tamanho - $tail_size) / $chunk_size; 
	...
	// problem - does not zero-fill the dia and mes yet
	...
	{	
		...
		for ($i=0; $i < $tamanho; $i++) {
			...
			case 'Y':
			case 'y':
				$s .= "extract(ano from $col)";
				break;
			case 'M':
			case 'm':
				$s .= "extract(mes from $col)";
				break;
			case 'Q':
			case 'q':
				$s .= "cast(((extract(mes from $col)+2) / 3) as integer)";
				break;
			case 'D':
			case 'd':
				$s .= "(extract(dia from $col))";
				break;
			case 'H':
			case 'h':
			  $s .= "(extract(hora from $col))";
			  break;                        
			case 'I':
			case 'i':
			  $s .= "(extract(minuto from $col))";
			  break;                
			case 'S':
			case 's':
			  $s .= "CAST((extract(segundo from $col)) AS INTEGER)";
			  break;        
...
class ADORecordset_ibase extends ADORecordSet
{
	...
	function ADORecordset_ibase($id,$modo=false)
	{
	global $ADODB_FETCH_MODE;
	
			$this->fetchMode = ($modo === false) ? $ADODB_FETCH_MODE : $modo;
