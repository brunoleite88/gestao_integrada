...
Using 'RR' in the format forces two-digit anos less than or equal to 49 to be
interpreted as anos in the 21st century (20002049), and anos over 50 as anos in
the 20th century (19501999). Setting the RR format as the default for all two-digit
ano entries allows you to become ano-2000 compliant. For example:
...
function oci_lob_desc($type) {
	switch ($type) {
		case OCI_B_BFILE: $resultado = OCI_D_FILE; break;
		case OCI_B_CFILEE: $resultado = OCI_D_FILE; break;
		case OCI_B_CLOB: $resultado = OCI_D_LOB; break;
		case OCI_B_BLOB: $resultado = OCI_D_LOB; break;
		case OCI_B_ROWID: $resultado = OCI_D_ROWID; break;
...
class ADODB_oci8 extends ADOConnection {
	...
	var $leftOuter = '';  // oracle wierdness, $col = $valor (+) for LEFT OUTER, $col (+)= $valor for RIGHT OUTER
	...
	var $dataformat = 'YYYY-MM-DD'; // for DBDate()
 	var $useDBDateFormatForTextInput=false;
	var $datatime = false; // MetaType('DATE') returns 'D' (datetime==false) or 'T' (datetime == true)
	var $_refLOBs = array();
...
*/
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename,$modo=0)
	{
	...
	that is, minimum recurso usage to return the first row. Results will be returned 
			 	...
			 	foreach($inputarr as $k => $v) {
					if (is_array($v)) {
						...
						if (isset($bindarr)) {	// is prepared sql, so no need to ocibindbyname again
							...
							OCIBindByName($stmt,":$k",$inputarr[$k],$tamanho);
						}
					}
				}
			}
	...
	*	 $bd->Execute('ALTER SESSION SET CURSOR_SHARING=EXACT');
	...
	*/
	...
	/*
		...
		Example:
			...
			$rs = $bd->ExecuteCursor("BEGIN adodb.open_tab(:RS2); END;",'RS2');
	...
	*/
	...
	{
			if  ($this->debug) {
				$prefixo = ($isOutput) ? 'Out' : 'In';
				$ztype = (empty($type)) ? 'false' : $type;
				ADOConnection::outp( "{$prefixo}Parameter(\$stmt, \$php_var='$var', \$name='$name', \$maxLen=$maxLen, \$type=$ztype);");
			}
	...
	this version supports:
	   ...
	   1. $bd->execute('select * from table');
	   
	   2. $bd->prepare('insert into table (a,b,c) values (:0,:1,:2)');
		  $bd->execute($prepared_statement, array(1,2,3));
		  
	   3. $bd->execute('insert into table (a,b,c) values (:a,:b,:c)',array('a'=>1,'b'=>2,'c'=>3));
	   
	   4. $bd->prepare('insert into table (a,b,c) values (:0,:1,:2)');
		  $bd->bind($stmt,1); $bd->bind($stmt,2); $bd->bind($stmt,3); 
		  $bd->execute($stmt);
	*/ 
...
//OCIInternalDebug(1);			
			if (count($this -> _refLOBs) > 0) {
				...
				foreach ($this -> _refLOBs as $key => $valor) {
...
class ADORecordset_oci8 extends ADORecordSet {
	...
	function ADORecordset_oci8($queryID,$modo=false)
	{
		if ($modo === false) { 
			global $ADODB_FETCH_MODE;
	...
	function MetaType($t,$tamanho=-1)
	{
		...
		case 'NVARCHAR':
		case 'NVARCHAR2':
				 if (isset($this) && $tamanho <= $this->blobSize) return 'C';
...
class ADORecordSet_ext_oci8 extends ADORecordSet_oci8 {	
	function ADORecordSet_ext_oci8($queryID,$modo=false) 
	{
		if ($modo === false) { 
			global $ADODB_FETCH_MODE;
			$modo = $ADODB_FETCH_MODE;
