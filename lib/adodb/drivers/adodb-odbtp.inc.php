...
class ADODB_odbtp extends ADOConnection{
	...
	// the same scope. A scope is a modulo -- a stored procedure, trigger,
	...
	function CreateSequence($seqname='adodbseq',$start=1)
	{
		...
		if( $this->odbc_driver == ODB_DRIVER_FOXPRO ) {
			$caminho = @odbtp_get_attr( ODB_ATTR_DATABASENAME, $this->_connectionID );
			//if using vfp dbc file
			if( !strcasecmp(strrchr($caminho, '.'), '.dbc') )
                $caminho = substr($caminho,0,strrpos($caminho,'\/'));
           	$seqtab = $caminho . '/' . $seqtab;
        }
		if($num == false) {
	...
	function &MetaTables($ttype='',$mostrarSchema=false,$mask=false)
	{
	global $ADODB_FETCH_MODE;
		...
		for ($i=0; $i < sizeof($arr); $i++) {
			...
			if ($arr[$i][2])
				$arr2[] = $mostrarSchema && $arr[$i][1]? $arr[$i][1].'.'.$arr[$i][2] : $arr[$i][2];
		}
		return $arr2;
	}
	...
	/*
	Usage:
		$stmt = $bd->PrepareSP('SP_RUNSOMETHING'); -- takes 2 params, @myid and @group

		# note that the parameter does not have @ in front!
		$bd->Parameter($stmt,$id,'myid');
		$bd->Parameter($stmt,$group,'group',false,64);
		$bd->Parameter($stmt,$group,'photo',false,100000,ODB_BINARY);
		$bd->Execute($stmt);
...
class ADORecordSet_odbtp extends ADORecordSet {
	...
	function ADORecordSet_odbtp($queryID,$modo=false)
	{
		if ($modo === false) {
			global $ADODB_FETCH_MODE;
	...
	function &FetchField($fieldOffset = 0)
	{
		...
		$o->name = @odbtp_campo_nome($this->_queryID,$off);
	...
	function fields($colname)
	{
		...
		if (!$this->bind) {
			...
			for ($i=0; $i < $this->_numOfFields; $i++) {
				$name = @odbtp_campo_nome( $this->_queryID, $i );
...
class ADORecordSet_odbtp_mssql extends ADORecordSet_odbtp {
	...
	function ADORecordSet_odbtp_mssql($id,$modo=false)
	{
		return $this->ADORecordSet_odbtp($id,$modo);
	}
}
...
class ADORecordSet_odbtp_access extends ADORecordSet_odbtp {
	...
	function ADORecordSet_odbtp_access($id,$modo=false)
	{
		return $this->ADORecordSet_odbtp($id,$modo);
	}
}
...
class ADORecordSet_odbtp_vfp extends ADORecordSet_odbtp {
	...
	function ADORecordSet_odbtp_vfp($id,$modo=false)
	{
		return $this->ADORecordSet_odbtp($id,$modo);
	}
}
...
class ADORecordSet_odbtp_oci8 extends ADORecordSet_odbtp {
	...
	function ADORecordSet_odbtp_oci8($id,$modo=false)
	{
		return $this->ADORecordSet_odbtp($id,$modo);
	}
}
...
class ADORecordSet_odbtp_sybase extends ADORecordSet_odbtp {
	...
	function ADORecordSet_odbtp_sybase($id,$modo=false)
	{
		return $this->ADORecordSet_odbtp($id,$modo);
	}
}
