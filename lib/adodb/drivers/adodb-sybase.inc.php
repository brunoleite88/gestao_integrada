...
class ADODB_sybase extends ADOConnection {
	...
	// see http://sybooks.sybase.com/onlinebooks/group-aw/awg0800e/dbrfen8/@ebt-link;pt=5981;uf=0?target=0;window=new;mostrartoc=true;book=dbrfen8
	...
	*/
    ...
    # Used ASA SQL Reference Manual -- http://sybooks.sybase.com/onlinebooks/group-aw/awg0800e/dbrfen8/@ebt-link;pt=5981;uf=0?target=0;window=new;mostrartoc=true;book=dbrfen8
    # to convert similar Microsoft SQL*Server (mssql) API into Sybase compatible version
...
class ADORecordset_sybase extends ADORecordSet {	
	...
	function ADORecordset_sybase($id,$modo=false)
	{
		if ($modo === false) { 
			global $ADODB_FETCH_MODE;
			$modo = $ADODB_FETCH_MODE;
		}
		if (!$modo) $this->fetchMode = ADODB_FETCH_ASSOC;
		else $this->fetchMode = $modo;
		$this->ADORecordSet($id,$modo);
	}
...
class ADORecordSet_array_sybase extends ADORecordSet_array {
	...
	global $ADODB_sybase_mths;
		...
		//Changed [0-9] to [0-9 ] in dia conversion
