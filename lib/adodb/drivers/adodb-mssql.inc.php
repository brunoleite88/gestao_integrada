...
// mes strings depends on what language has been configured. The 
...
//
// Also if your mes is mostraring as mes-1, 
//   e.g. Jan 13, 2002 is mostraring as 13/0/2002, then see
...
// docs say 4.2.0, but testing mostrars only since 4.3.0 does it work!
	ini_set('mssql.datetimeconvert',0); 
} else {
global $ADODB_mssql_mths;		// array, mess must be upper-case
...
class ADORecordset_mssql extends ADORecordSet {	
	...
	function ADORecordset_mssql($id,$modo=false)
	{
		// freedts check...
		...
		$this->fetchMode = $modo;
		return $this->ADORecordSet($id,$modo);
	}
...
class ADORecordSet_array_mssql extends ADORecordSet_array {
	function ADORecordSet_array_mssql($id=-1,$modo=false) 
	{
		$this->ADORecordSet_array($id,$modo);
	}
	...
	global $ADODB_mssql_mths,$ADODB_mssql_date_order;
		...
		// h-m-s-MM-DD-YY
		return  mktime(0,0,0,$themth,$thedia,$rr[3]);
	}
	...
	global $ADODB_mssql_mths,$ADODB_mssql_date_order;
		...
		// h-m-s-MM-DD-YY
		return  mktime($rr[4],$rr[5],0,$themth,$thedia,$rr[3]);
	}
}
