...
/* 
  ...
  probably still some remnants of the ODBC support in this file; I'm relying on revers
...
class ADODB_db2 extends ADOConnection {
	...
	// use right() and replace() ?
		...
		for ($i=0; $i < $tamanho; $i++) {
			...
			case 'Y':
			case 'y':
				if ($tamanho==1) return "ano($col)";
				$s .= "char(ano($col))";
				break;
			case 'M':
				if ($tamanho==1) return "mesname($col)";
				$s .= "substr(mesname($col),1,3)";
				break;
			case 'm':
				if ($tamanho==1) return "mes($col)";
				$s .= "right(digits(mes($col)),2)";
				break;
			case 'D':
			case 'd':
				if ($tamanho==1) return "dia($col)";
				$s .= "right(digits(dia($col)),2)";
				break;
			case 'H':
			case 'h':
				if ($tamanho==1) return "hora($col)";
				if ($col != $this->sysDate) $s .= "right(digits(hora($col)),2)";	
				else $s .= "''";
				break;
			case 'i':
			case 'I':
				if ($tamanho==1) return "minuto($col)";
				if ($col != $this->sysDate)
			...
			case 'S':
			case 's':
				if ($tamanho==1) return "segundo($col)";
				if ($col != $this->sysDate)
	...
	function ServerInfo()
	{
		...
		if (!empty($this->host) && ADODB_PHPVER >= 0x4300) {
			...
			if (!$encontrado) return ADOConnection::ServerInfo();
...
class ADORecordSet_db2 extends ADORecordSet {	
	...
	function &FetchField($offset = -1) 
	{
		...
		$o->name = @db2_campo_nome($this->_queryID,$offset);
