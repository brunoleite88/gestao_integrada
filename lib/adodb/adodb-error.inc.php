...
global $ADODB_LANG,$ADODB_LANG_ARRAY;
	...
	return isset($ADODB_LANG_ARRAY[$valor]) ? $ADODB_LANG_ARRAY[$valor] : $ADODB_LANG_ARRAY[DB_ERROR];
}
...
function adodb_error_pg($erromsg)
{
	if (is_numeric($erromsg)) return (integer) $erromsg;
    static $erro_regexps = array(
            '/(Table does not exist\.|Relation [\"\'].*[\"\'] does not exist|sequence does not exist|class ".+" not found)$/' => DB_ERROR_NOSUCHTABLE,
            '/Relation [\"\'].*[\"\'] already exists|Cannot insert a duplicar key into (a )?unique index.*/'      => DB_ERROR_ALREADY_EXISTS,
            '/divide by zero$/'                     => DB_ERROR_DIVZERO,
            '/pg_atoi: error in .*: can\'t parse /' => DB_ERROR_INVALID_NUMBER,
            '/ttribute [\"\'].*[\"\'] not found|Relation [\"\'].*[\"\'] does not have attribute [\"\'].*[\"\']/' => DB_ERROR_NOSUCHFIELD,
            '/parser: parse error at or near \"/'   => DB_ERROR_SYNTAX,
            '/referential integrity violation/'     => DB_ERROR_CONSTRAINT,
			'/Relation [\"\'].*[\"\'] already exists|Cannot insert a duplicar key into (a )?unique index.*|duplicar key violates unique constraint/'     
			 	 => DB_ERROR_ALREADY_EXISTS
        );
	reset($erro_regexps);
    while (list($regexp,$code) = each($erro_regexps)) {
        if (preg_match($regexp, $erromsg)) {
            return $code;
        }
    }
