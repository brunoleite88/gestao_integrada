...
class ADODB_Pager {
	...
	var $bd; 	// ADODB connection object
	var $sql; 	// sql used
	var $rs;	// recordset generated
	var $curr_page;	// current page number before Render() called, calculated in constructor
	var $rows;		// number of rows per page
    var $linksPerPage=10; // number of links per page in navigation bar
    var $mostrarPageLinks; 
	...
	// constructor
	//
	// $bd	adodb connection object
	// $sql	sql statement
	// $id	optional id to identify which pager, 
	//		if you have multiple on 1 page. 
	//		$id should be only be [a-z0-9]*
	//
	function ADODB_Pager(&$bd,$sql,$id = 'adodb', $mostrarPageLinks = false)
	{
	global $PHP_SELF;
	
		$curr_page = $id.'_curr_page';
		if (empty($PHP_SELF)) $PHP_SELF = htmlspecialchars(previnirXSS($_SERVER['PHP_SELF'])); // htmlspecialchars() to prevento XSS attacks
		
		$this->sql = $sql;
		$this->id = $id;
		$this->db = $bd;
		$this->mostrarPageLinks = $mostrarPageLinks;
	...
	{
		if (!$this->db->pageExecuteCountRows) return '';
		$ultimoPage = $this->rs->LastPageNo();
		if ($ultimoPage == -1) $ultimoPage = 1; // check for empty rs.
		if ($this->curr_page > $ultimoPage) $this->curr_page = 1;
		return "<font size=-1>$this->page ".$this->curr_page."/".$ultimoPage."</font>";
	}
