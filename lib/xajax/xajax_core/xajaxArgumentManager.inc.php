...
//				$sText .= $sArg[$nCurrent];
	...
	function argumentDecodeUTF8_gpw_passthrough(&$mArg)
	{
		if (is_array($mArg))
		{
			foreach (array_keys($mArg) as $sKey)
			{
				$sNewKey = $sKey;
				$this->argumentDecodeUTF8_gpw_passthrough($sNewKey);
				...
				$this->argumentDecodeUTF8_gpw_passthrough($mArg[$sKey]);
			}
		}
		else if (is_string($mArg))
			$mArg = gpw_passthrough($mArg);
	}
