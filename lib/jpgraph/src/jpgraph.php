    ...
    if (!defined('CACHE_DIR')) {
        if ( strstr( PHP_OS, 'WIN') ) {
            if( empty($_SERVER['TEMP']) ) {
                ...
                define('CACHE_DIR', previnirXSS($_SERVER['TEMP']) . '/');
            }
        } else {
...
function GenImgName() {
    ...
    $fname = basename(previnirXSS($_SERVER['PHP_SELF']));
    if( !empty($_SERVER['QUERY_STRING']) ) {
        $q = previnirXSS(@$_SERVER['QUERY_STRING']);
...
class Graph {
    ...
    function CheckCSIMCache($aCacheName,$aTimeOut=60) {
        ...
        if( $aCacheName=='auto' ) {
            $aCacheName=basename(previnirXSS($_SERVER['PHP_SELF']));
        }
    ...
    static function GetURLArguments($aAddRecursiveBlocker=false) {
        ...
        while( list($key,$value) = previnirXSS(each($_POST)) ) {
            if( is_array($value) ) {
                foreach ( $value as $k => $v ) {
                    $urlarg .= '&amp;'.$key.'%5B'.$k.'%5D='.urlencode($v);
                }
    ...
    function StrokeCSIM($aScriptName='auto',$aCSIMName='',$aBorder=0) {
        ...
        if( $aScriptName=='auto' ) {
            $aScriptName=basename(previnirXSS($_SERVER['PHP_SELF']));
        }
