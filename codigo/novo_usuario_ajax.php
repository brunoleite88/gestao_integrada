...
function selecionar_secao_ajax($dept_id=1){
	...
	foreach($linhas as $linha) {
		$vetor[$linha['dept_id']]=gpw_passthrough($linha['dept_nome']);
		$sql->adTabela('depts');
		$sql->adCampo('depts.dept_id, depts.dept_nome');
		$sql->adOnde('depts.dept_cia='.(int)$dept_id);
		...
		$sql->limpar();
		foreach($subordinadas as $subordinada) $vetor[$subordinada['dept_id']]=gpw_passthrough('&nbsp;&nbsp;&nbsp;&nbsp;'.$subordinada['dept_nome']);	
		}
...
function existe_login_ajax($login=''){
	...
	$sql->adOnde('usuario_login=\''.previnirXSS(gpw_passthrough($login)).'\'');
...
function existe_identidade_ajax($identidade=''){
	if (!$identidade) {
		...
		$sql->adOnde('contato_identidade=\''.previnirXSS(gpw_passthrough($identidade)).'\'');
		$existe=$sql->Resultado();
		$sql->limpar();
		$objResposta = new xajaxResponse();
