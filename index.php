...
Gestão Estratégica Integrada\index.php

Arquivo base do aplicativo, do qual os outros arquivos são incluidos

convenções:

	$m é o nome da subpasta de modulos
...
if (is_file(BASE_DIR.'/config.php')) require_once BASE_DIR.'/config.php';
	...
	echo '<html><head><meta http-equiv="refresh" content="5; URL='.BASE_URL.'/index.php" charset="utf-8"><LINK REL="SHORTCUT ICON" href="estilo/rondon/imagens/organizacao/10/favicon.ico"><link rel="stylesheet" type="text/css" href="estilo/rondon/estilo_'.(isset($config['estilo_css']) ? $config['estilo_css'] : 'metro').'.css"></head><body>';
	echo '<br><br><br>';
...
if ($Aplic->fazerLogin()) {
	...
	if (isset($localidade_tipo_caract)) header('Content-type: text/html;charset=utf-8');
...
if (!isset($_REQUEST['m'])) {
	...
	$m =(isset($Aplic->usuario_prefs['padrao_ver_m']) && $Aplic->usuario_prefs['padrao_ver_m'] ? $Aplic->usuario_prefs['padrao_ver_m'] : (isset($config['padrao_ver_m']) ? $config['padrao_ver_m'] : 'projetos'));
	$def_a =(isset($Aplic->usuario_prefs['padrao_ver_a']) && $Aplic->usuario_prefs['padrao_ver_a'] ? $Aplic->usuario_prefs['padrao_ver_a'] : (isset($config['padrao_ver_a']) ? $config['padrao_ver_a']: 'index'));
	$tab =(isset($Aplic->usuario_prefs['padrao_ver_tab']) && $Aplic->usuario_prefs['padrao_ver_tab']!='' ? $Aplic->usuario_prefs['padrao_ver_tab'] : (isset($config['padrao_ver_tab']) ? $config['padrao_ver_tab'] : 0));
...
if (!$sem_cabecalho && isset($localidade_tipo_caract)) header('Content-type: text/html;charset=utf-8');
...
if(!$sem_cabecalho){
    if($Aplic->profissional){
        echo '<script>';
        echo 'if(parent && parent.geiApp){var geiApp = parent.geiApp;}';
        echo 'if(geiApp){';
        if($logou){
            $dados = array( 'id' => $Aplic->usuario_id, 'nome' => ($Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra));
            ...
            echo 'geiApp.onLogin(dados_usu);';
            }
            echo 'geiApp.setAppVersao("'.$Aplic->getVersao().'","'.$Aplic->getVersaoJs().'");';
            echo 'geiApp.setCurrent('.json_encode(toUtf8($_REQUEST)).');';
            echo 'geiApp.module = "'.$m.'";';
            echo 'geiApp.sub_module = "'.$u.'";';
            echo 'geiApp.arquive = "'.$a.'";';
            echo 'geiApp.administrator = '.(($Aplic->usuario_admin || $Aplic->usuario_super_admin) ? 'true' : 'false').';';
            echo '}</script>';
        }
    echo '</body></html>';
    }
